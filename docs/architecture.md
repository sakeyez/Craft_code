...
A **profile** is a named composition stored in the Harness home. It lists the bundles it stacks, holds any out-of-tree plugins it installs, and keeps the user's own `cordis.patch.yml`. `web`, `headless`, and `mcmod` ship as templates.
...
[`dsh-base`](../packages/bundle/base/README.md) is the first layer of every profile: model adapters, tools, persistence, sandbox and approval policy, settings, credentials, telemetry. [`dsh-desktop-app`](../packages/bundle/desktop-app/README.md) adds the private Electron renderer and loopback HTTP carrier; [`dsh-headless`](../packages/bundle/headless/README.md) adds a one-shot runner with no server at all. [`dsh-mcmod-headless-bundle`](../packages/bundle/mcmod-headless/README.md) specializes that headless runner for Fabric and NeoForge Minecraft Java mod-development tasks.
