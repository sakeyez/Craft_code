...
**profile** 是存放在 Harness home 中的具名组装。它列出自己叠放的组合包，存放自己安装的树外插件，并保存用户自己的 `cordis.patch.yml`。`web`、`headless` 与 `mcmod` 作为模板随发行版交付。
...
[`dsh-base`](../packages/bundle/base/README.zh.md) 是每个 profile 的第一层：模型适配器、工具、持久化、沙箱与审批策略、设置、凭据、遥测。[`dsh-desktop-app`](../packages/bundle/desktop-app/README.zh.md) 增加私有 Electron renderer 与 loopback HTTP 载体；[`dsh-headless`](../packages/bundle/headless/README.zh.md) 增加一次性运行器，且完全不带服务器。[`dsh-mcmod-headless-bundle`](../packages/bundle/mcmod-headless/README.zh.md) 将该 headless runner 专门化为 Fabric 与 NeoForge Minecraft Java 模组开发任务入口。
