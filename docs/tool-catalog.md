...
| `@deepseek-ai/dsh-tool-mc-project` | `detect_mc_project`, `validate_mc_resources` | `ctx.tools`, `ctx.fs` | `tool/call`, `tool/result` | - | Minecraft project tools read the current workspace through ctx.fs. They extract Gradle, metadata, and resource facts without executing Gradle, loading dependencies, or emulating Minecraft resource loading. |
