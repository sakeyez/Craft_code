...
- Your patch can override rows from earlier layers by `id` — the same way [the `dsh-desktop-app` bundle](../../../../packages/bundle/desktop-app/cordis.patch.yml) overrides `dsh-base` rows — but must restate every key the row needs, not just the changed one.
