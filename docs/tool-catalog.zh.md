...
| `@deepseek-ai/dsh-tool-mc-project` | `detect_mc_project`、`validate_mc_resources` | `ctx.tools`、`ctx.fs` | `tool/call`、`tool/result` | - | Minecraft 项目工具通过 ctx.fs 读取当前 workspace。它们提取 Gradle、metadata 与 resource 事实，不执行 Gradle，不加载依赖，也不模拟 Minecraft 资源加载。 |
...
用确定性静态检查校验当前 Minecraft mod workspace 资源：lang JSON 语法、item/block model texture 引用、blockstate model 引用、recipe/tag JSON 语法、可疑 namespace，以及 mod id 与 metadata 一致性。它不执行 Gradle，也不模拟 Minecraft 资源加载。
...
Minecraft 项目工具通过 ctx.fs 读取当前 workspace。它们提取 Gradle、metadata 与 resource 事实，不执行 Gradle，不加载依赖，也不模拟 Minecraft 资源加载。
