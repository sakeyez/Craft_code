...
The runner supports `build`, `test`, `datagen`, `resources`, `runtime`, and `all`. Fabric and NeoForge are supported for loader-specific datagen/runtime task selection; Forge and Quilt remain diagnostic-only and fail closed before those tasks. Unknown or conflicting loader evidence also fails before loader-specific execution. `resources` runs the static `validate_mc_resources` check before Gradle `processResources`, and `all` runs datagen only when supported detection found datagen clues, then resources, test, and build, stopping at the first failed step. The support classification is maintained in [the loader-scope note](2026-08-23-minecraft-loader-support-scope.md).
