...
**Run Gradle tasks to discover project state.** Rejected for the detector because Gradle can download dependencies, run arbitrary build logic, and take much longer than a cheap first read. The tool recommends validation commands; the shell-backed check runner uses the detected facts when the agent needs to execute validation.
