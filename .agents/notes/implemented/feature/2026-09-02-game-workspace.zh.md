...
标注通过隔离 renderer 中预热的 10 FPS 无音频流捕获已验证且可见的 Minecraft 窗口。枚举时不请求缩略图；仅在截图请求时将新鲜静帧编码为质量 75、不超过 1920x1080 的 JPEG 传给主进程，失败则回退到缩略图路径。事务持有静帧直到结束。归一化几何和会话授权的图片附件通过 `session.annotate` 持久化。捕获不改变 Minecraft 的 parent、样式或几何。
...
标注事务在已验证的 Minecraft 客户区复用隐藏、预加载的覆盖层。完成和取消会隐藏覆盖层并清除草稿与像素；几何失效、所属页面导航和退出会释放覆盖层。每条覆盖层 IPC 携带操作编号，迟到消息不能提交或取消后续事务。Ctrl+Shift+P 使用系统注册，因为 renderer 无法观察游戏获得焦点时的按键；前台检查将触发范围限制为已持有的游戏或 CraftCode。当前会话负责注册和清理。

## 考虑过的替代方案

**用 HTML 视频替代原生游戏交互。** 对每一帧编码和传输会增加延迟、GPU 与 CPU 成本，还需要单独的输入转发协议。外部原生窗口保持 GLFW 渲染和直接 Windows 输入。
...
**每次点击才启动捕获。** 逐窗口生成缩略图和启动 renderer 会延迟静帧显示。私有无音频流与可复用覆盖层接受后台资源开销，以免重复支付这些成本。不需要连续编码、存储、音频或输入转发。

## 结果

真实 Electron 捕获探针记录冷启动／预热延迟和可见像素。单元测试覆盖操作范围内取消、窗口复用、前台检查、快捷键占用和长按。真实 Minecraft 帧率／GPU 影响及多显示器游戏行为仍是独立的运行验收缺口。
