...
- `pnpm run verify-node-next-types` scans built declarations for relative specifiers without file extensions, then typechecks a temporary external ESM consumer with `moduleResolution: "NodeNext"` against the built `types`/`exports` surface, so declaration specifier regressions fail before publish. The temporary consumer links workspace package directories into its `node_modules`; Windows uses junctions so the check needs no symlink privilege, and cleanup unlinks every directory link before recursively removing the temporary tree.
