...
- `apps/` holds the externally exported applications, assembled from Client / Host mixtures.
    ...
    - `apps/desktop` reuses `dsh web` through its loopback HTTP/WebSocket carrier; a future packaged shell may replace only that carrier when it has a concrete need ([desktop-shell decision](2026-08-28-electron-loopback-desktop-shell.md)).
...
| Carrier layer | `dsh-host-webserver` | Web HTTP and upgrade: static serving + `/api/*`→handler forwarding + WebSocket upgrade route + close semantics; plugin bundle endpoint + `__DSH_BOOT__` manifest injection (fed by the web plugin registry) | Browser access and the loopback Electron development shell; zero workspace dependencies (the registry arrives by structural injection) |
...
1. **Pick a fetch impersonation**: browser or desktop-shell same-origin HTTP / in-process `host.handler.fetch` injection / your own transport-aspect subclass (e.g. a future packaged-shell IPC carrier, see the "Subclass table" below).
...
| IPC bridge subclass (hypothetical example — the development shell uses loopback HTTP) | a packaged Electron shell | IPC serialization round trip | would swap only doFetch; contract and base class unchanged |
