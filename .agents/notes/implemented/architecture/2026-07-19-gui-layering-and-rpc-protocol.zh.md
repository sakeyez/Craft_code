...
- `apps/` 作为对外导出的应用入口，可以由 Client / Host 混合组装。
    ...
    - `apps/desktop` 经由 `dsh web` 的 loopback HTTP/WebSocket 载体复用同一套 web client 包；未来的打包桌面壳只有在存在具体需求时才替换该载体（见[桌面壳决策](2026-08-28-electron-loopback-desktop-shell.zh.md)）。
...
| 承载层 | `dsh-host-webserver` | Web HTTP 与 upgrade：静态服务 + `/api/*`→handler 转发 + WebSocket upgrade route + close 语义；插件 bundle 端点 + `__DSH_BOOT__` manifest（元数据清单）注入（由 web 插件注册表供给） | 浏览器访问与 loopback Electron 开发壳；零 workspace 依赖（注册表经结构注入到达） |
...
1. **选 fetch 伪造方式**：浏览器或桌面壳同源 HTTP / 进程内 `host.handler.fetch` 注入 / 自写传输切面子类（如未来打包桌面壳的 IPC 载体，见下文「子类表」）。
...
| IPC 桥子类（假想示例——开发壳使用 loopback HTTP） | 打包后的 Electron 壳 | IPC 序列化往返 | 只需换 doFetch，约定/基类零改 |
