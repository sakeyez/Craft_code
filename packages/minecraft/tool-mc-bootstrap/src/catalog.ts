...
import {
  ...
  isSupportedNeoForgeMinecraftVersion,
  jdkForMinecraft,
} from './validation.ts'
...
interface FabricGame { version?: unknown; stable?: unknown }
interface FabricLoaderVersion {
  loader?: { version?: unknown; stable?: unknown }
  intermediary?: { version?: unknown }
  yarn?: { version?: unknown; build?: unknown }
}
...
interface GitHubRepository { name?: unknown }
...
export function parseFabricGameVersions(value: unknown): string[] {
  if (!Array.isArray(value)) return []
  return [...new Set(value.flatMap((item) => {
    if (typeof item !== 'object' || item === null) return []
    const row = item as FabricGame
...
export function parseFabricLoaderVersions(value: unknown): Array<{ loader: string; intermediary: string }> {
  if (!Array.isArray(value)) return []
  return value.flatMap((item) => {
    if (typeof item !== 'object' || item === null) return []
    const row = item as FabricLoaderVersion
    const loader = row.loader?.version
    const intermediary = row.intermediary?.version
    if (typeof loader !== 'string' || row.loader?.stable !== true || typeof intermediary !== 'string') return []
...
export function parseFabricYarnVersions(value: unknown, minecraftVersion: string): string[] {
  if (!Array.isArray(value)) return []
  return value.flatMap((item) => {
    if (typeof item !== 'object' || item === null) return []
    const row = item as FabricYarnVersion
    if (row.gameVersion !== minecraftVersion || typeof row.version !== 'string') return []
...
export function parseFabricApiVersion(xml: string, minecraftVersion: string): string | undefined {
...
export function parseNeoForgeMetadata(xml: string): Array<{ minecraftVersion: string; loaderVersion: string }> {
...
export function parseNeoForgeDirectoryIndex(html: string): Array<{ minecraftVersion: string; loaderVersion: string }> {
...
export function parseNeoForgeMdkRecipe(buildGradle: string, wrapperProperties: string): NeoForgeMdkRecipe | undefined {
  ...
  if (pluginVersion === undefined || gradleVersion === undefined || gradleRecipe(gradleVersion) === undefined) return undefined
  return { pluginVersion, gradleVersion }
}

/** Extract supported NeoGradle MDK game lines from the official GitHub organization. */
export function parseNeoForgeMdkRepositories(value: unknown): string[] {
  if (!Array.isArray(value)) return []
  const versions = value.flatMap((item) => {
    if (typeof item !== 'object' || item === null) return []
    const name = (item as GitHubRepository).name
...
export function parseNeoForgeMdkLoader(properties: string, minecraftVersion: string): string | undefined {
  const loaderVersion = /^neo_version\s*=\s*(\d+(?:\.\d+){2,3})\s*$/mu.exec(properties)?.[1]
  if (loaderVersion === undefined) return undefined
  return parseNeoForgeVersions([loaderVersion])[0]?.minecraftVersion === minecraftVersion ? loaderVersion : undefined
}

function parseNeoForgeVersions(versions: readonly string[]): Array<{ minecraftVersion: string; loaderVersion: string }> {
  const out: Array<{ minecraftVersion: string; loaderVersion: string }> = []
  const seen = new Set<string>()
  for (const loaderVersion of versions) {
    if (/(?:alpha|beta|rc|snapshot)/iu.test(loaderVersion)) continue
    ...
    if (!isSupportedNeoForgeMinecraftVersion(minecraftVersion)) continue
...
export function fabricEntry(
  minecraftVersion: string,
  loaderVersion: string,
  intermediary: string,
  yarn: string,
  apiVersion = `0.102.0+${minecraftVersion}`,
): CatalogEntry {
...
export function neoForgeEntry(
  minecraftVersion: string,
  loaderVersion: string,
  recipe?: NeoForgeMdkRecipe,
): CatalogEntry {
...
export function normalizeCatalogEntries(entries: readonly CatalogEntry[]): CatalogEntry[] {
...
export class MinecraftCatalogResolver {
  ...
  private readonly doFetch: CatalogFetch
  private readonly now: () => Date
  private readonly probeJava: JavaProbeFn
   ...
   */
  async load(signal?: AbortSignal, refresh = true): Promise<CatalogSnapshot> {
    const java = await this.probeJava(signal).catch((error: unknown) => ({
      available: false,
    ...
    try {
      ...
      if (resolvedLoaders.size < 2) {
        const missing = (['fabric', 'neoforge'] as const).filter(loader => !resolvedLoaders.has(loader))
      ...
      try { await writeCache(this.cachePath, { format: 1, fetchedAt, entries }) }
      catch (error) { cacheError = error instanceof Error ? error.message : String(error) }
      return {
   ...
   */
  async cached(): Promise<CatalogSnapshot | undefined> {
    const cached = await readCache(this.cachePath)
  ...
  private async fetchOnlineWithinBudget(signal?: AbortSignal): Promise<CatalogEntry[]> {
  ...
  private async fetchOnline(signal?: AbortSignal): Promise<CatalogEntry[]> {
  ...
  private async fetchFabricEntries(signal?: AbortSignal): Promise<CatalogEntry[]> {
    ...
    return resolved.filter((entry): entry is CatalogEntry => entry !== undefined)
  }

  private async fetchNeoForgeEntries(signal?: AbortSignal): Promise<CatalogEntry[]> {
    ...
    const publishedByMinecraft = new Map<string, Set<string>>()
    for (const row of neoRows) {
      const published = publishedByMinecraft.get(row.minecraftVersion) ?? new Set<string>()
    ...
    const resolved = await mapWithConcurrency(minecraftVersions, 6, minecraftVersion => this.resolveNeoForgeMdk(
      minecraftVersion,
      publishedByMinecraft.get(minecraftVersion),
      signal,
    ))
    return resolved.filter((entry): entry is CatalogEntry => entry !== undefined)
  }

  private async resolveNeoForgeMdk(
    minecraftVersion: string,
    publishedLoaderVersions: ReadonlySet<string> | undefined,
    signal?: AbortSignal,
    ...
    if (recipe === undefined || loaderVersion === undefined || !metadata.includes('[[mods]]')) return undefined
    if (publishedLoaderVersions !== undefined && !publishedLoaderVersions.has(loaderVersion)) return undefined
   ...
   */
  private async fetchNeoForgeMdkRepositories(signal?: AbortSignal): Promise<unknown[]> {
    const repositories: unknown[] = []
    for (let page = 1; page <= GITHUB_MDK_MAX_PAGES; page += 1) {
      const suffix = page === 1 ? '' : `&page=${String(page)}`
      const raw = await this.getJson(
        `${GITHUB_MDK_REPOSITORIES_URL}?per_page=${String(GITHUB_MDK_PAGE_SIZE)}&type=public${suffix}`,
        signal,
      ).catch(() => undefined)
      ...
      if (repositories.length >= GITHUB_MDK_PAGE_SIZE * GITHUB_MDK_MAX_PAGES
        || rows.length < GITHUB_MDK_PAGE_SIZE) break
    }
    return repositories
  }
  ...
  private async getFabricYarnVersion(minecraftVersion: string, signal?: AbortSignal): Promise<string | undefined> {
  ...
  private async getFabricApiMetadata(signal?: AbortSignal): Promise<string> {
    const urls = [
    ...
    for (const url of urls) {
      ...
      if (/<version>\s*[^<\s]+\s*<\/version>/u.test(xml)) return xml
    }
    return ''
  }
  ...
  private async request(url: string, signal?: AbortSignal): Promise<Response> {
    ...
    try {
      return await boundedCatalogPromise(
        () => this.doFetch(url, { signal: controller.signal }),
        signal,
        controller,
      )
    } finally {
...
function boundedCatalogPromise<T>(
...
function gradleRecipe(version: string): typeof NEOFORGE_GRADLE_1202 | typeof NEOFORGE_GRADLE_MODERN | undefined {
  if (version === NEOFORGE_GRADLE_1202.version) return NEOFORGE_GRADLE_1202
  if (version === NEOFORGE_GRADLE_MODERN.version) return NEOFORGE_GRADLE_MODERN
  return undefined
}

/** Accept only recipes derived from this build's checksummed template policy. */
function isTrustedCatalogRecipe(entry: CatalogEntry): boolean {
...
export async function probeJava(signal?: AbortSignal): Promise<JavaProbe> {
  const executable = await findJavaExecutable()
  if (executable === undefined) return { available: false, message: '未在 PATH 中找到 Java' }
  return await new Promise((resolvePromise) => {
    ...
    let output = ''
    let settled = false
    const finish = (result: JavaProbe): void => {
      if (settled) return
      settled = true
      clearTimeout(timer)
      signal?.removeEventListener('abort', abort)
    ...
    child.once('close', (code) => {
      if (code !== 0) { finish({ available: false, message: output.trim().slice(-512) }); return }
...
function cleanProbeEnv(): NodeJS.ProcessEnv {
  ...
  for (const [key, value] of Object.entries(process.env)) {
    if (value !== undefined && !blocked.has(key) && !blockedPrefixes.some(prefix => key.startsWith(prefix))) env[key] = value
  }
  return env
}
...
async function readCache(path: string): Promise<CacheFile | undefined> {
  try {
    if (await hasSymlinkComponent(path)) return undefined
    const parsed = JSON.parse(await readFile(path, 'utf8')) as Partial<CacheFile>
    if (parsed.format !== 1 || typeof parsed.fetchedAt !== 'string' || !Array.isArray(parsed.entries)) return undefined
...
async function writeCache(path: string, cache: CacheFile): Promise<void> {
  ...
  if (existing?.isSymbolicLink()) throw new Error('catalog cache path cannot be a symbolic link')
  const temporary = `${path}.${process.pid}.${randomUUID()}.tmp`
  try {
    await writeFile(temporary, `${JSON.stringify(cache, null, 2)}\n`, { encoding: 'utf8' })
    if (await hasSymlinkComponent(directory)) throw new Error('catalog cache directory changed to a symbolic link')
