...
export const BOOTSTRAP_PROGRESS_EVENT = 'minecraft-bootstrap/progress' as const
...
export function jdkForMinecraft(version: string): 17 | 21 | undefined {
...
export function isSupportedMinecraftVersion(version: string): boolean {
...
export function isStableMinecraftVersion(version: string, stable = true): boolean {
 ...
 */
export function isSupportedNeoForgeMinecraftVersion(version: string): boolean {
  const normalized = version.trim()
  return normalized !== '1.20.1' && isStableMinecraftVersion(normalized)
}

/** Canonical, collision-resistant catalog id. */
export function catalogEntryId(loader: BootstrapLoader, minecraftVersion: string, loaderVersion: string): string {
...
export function deriveDirectoryName(modName: string, modId: string): string {
...
export function recordPayload(value: unknown): Record<string, unknown> | undefined {
...
export function isCatalogEntry(value: unknown): value is CatalogEntry {
  ...
  if (row.loader === 'neoforge' && !isSupportedNeoForgeMinecraftVersion(minecraftVersion)) return false
  ...
  if (row.loader === 'fabric') {
    if (mappingsVersion !== `${minecraftVersion}+build.${mappingsVersion.split('+build.')[1] ?? ''}`
      || !/^\d+(?:\.\d+){1,3}\+1\.2[01](?:\.\d+)?$/u.test(apiVersion)) return false
  ...
  if (row.requiredJdk !== jdkForMinecraft(minecraftVersion)) return false
  if (row.entryId !== catalogEntryId(row.loader, minecraftVersion, loaderVersion)) return false
