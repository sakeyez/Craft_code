...
export type OperationStage = 'validate' | 'java' | 'generate' | 'gradle-download' | 'dependencies' | 'build' | 'finalize' | 'done';
...
/** Response to `start`. */
export interface StartRpcResponse {
    readonly operationId: string;
}
/** JSON payload accepted by the status and cancel endpoints. */
export interface OperationRpcRequest {
    readonly operationId: string;
}
