...
import { ensureJava, networkFetch, networkJavaEnvironment, networkGradleArguments, isMinecraftNetworkFailure, automaticMinecraftNetwork } from '@deepseek-ai/dsh-mc-workbench';
...
import { isSha256, isValidDirectoryName, isValidModId, isValidModName, isValidPackageName, recordPayload, isCatalogEntry, } from "./validation.js";
import { MinecraftCatalogResolver } from "./catalog.js";
import { projectFiles, wrapperFiles } from "./template.js";
import { gradleDistributionDownloadFailed, gradleFailureMessage } from "./gradle-failure.js";
...
export function applyBootstrapService(ctx, options = {}) {
    const prepareJava = ctx.get('subprocess')
...
export class MinecraftBootstrapService {
    ctx;
    resolver;
    now;
    operations = new Map();
    ...
    async start(request, signal) {
        if (this.disposed)
            throw new Error('bootstrap service is disposed');
        if (this.activeOperation !== undefined)
            throw new Error('another Minecraft project is already being built');
        if (signal?.aborted)
        ...
        const operation = { controller, state };
    ...
    cancel(operationId) {
        ...
        if (operation === undefined || operation.controller.signal.aborted
            || ['ready', 'failed', 'cancelled'].includes(operation.state.snapshot.status))
            return false;
    ...
    async dispose() {
        ...
        for (const operation of this.operations.values()) {
            if (isTerminal(operation.state.snapshot.status))
                continue;
    ...
    async dispatchRpc(endpoint, payload, signal) {
        try {
            if (endpoint === 'catalog-cache')
                return { ok: true, value: await this.resolver.cached() ?? null };
            if (endpoint === 'catalog')
                return { ok: true, value: await this.catalog(signal) };
            if (endpoint === 'start')
                return { ok: true, value: await this.start(parseStartPayload(payload), signal) };
            if (endpoint === 'status') {
                const id = parseOperationPayload(payload).operationId;
                const snapshot = this.status(id);
                if (snapshot === undefined)
                    return errorResult(`unknown operation ${id}`);
    ...
    async run(operation, request) {
        ...
        try {
            ...
            const expectedHashes = Object.fromEntries(Object.entries(files).map(([path, content]) => [path, sha256(Buffer.from(content, 'utf8'))]));
            ...
            let resumable;
            try {
                resumable = await findResumableStaging(parent, finalPath, entry.entryId, requestFingerprint, expectedHashes);
            }
            catch (error) {
            ...
            const generatedHashes = { ...(resumable?.state.generatedHashes ?? {}) };
            for (const [path, content] of Object.entries(files)) {
                ...
                if (existing !== undefined) {
                    ...
                    if (actual !== expected)
                        throw stagingModified(staging, `生成文件已被修改: ${path}`);
                }
                else {
            ...
            await this.ensureWrapperJar(entry.gradleVersion, jar, controller.signal, append, entry.wrapperSha256);
            ...
            const build = this.options.build
                ?? ((path, selected, buildSignal, log, javaExecutable) => runGradleBuild(path, selected, buildSignal, log, this.cacheDirectory, javaExecutable));
            await build(staging, entry, controller.signal, append, java.executable);
            controller.signal.throwIfAborted();
            await assertStagingTree(staging, expectedHashes, true);
            ...
            if (cancelled) {
                ...
                this.fail(operation, errorCode(error), error instanceof Error ? error.message : String(error), staging);
            }
            if (staging !== undefined) {
                try {
                    await appendStateLog(staging, `FAILED: ${error instanceof Error ? error.message : String(error)}\n`);
                }
                catch (logError) {
                    this.update(operation, {
                        message: `${error instanceof Error ? error.message : String(error)}（完整日志写入失败：${logError instanceof Error ? logError.message : String(logError)}）`,
                    });
                }
            }
        }
        finally {
            if (this.activeOperation === operation.state.snapshot.operationId)
                this.activeOperation = undefined;
        }
    }
    logger(staging, operation) {
        const path = join(staging, '.dsh/bootstrap.log');
        return async (text) => {
            if (await containsSymlink(staging, path))
                throw bootstrapError('staging-modified', '日志路径不能包含符号链接');
    ...
    async ensureWrapperJar(version, destination, signal, append, expectedSha256) {
        ...
        for (const [index, url] of urls.entries()) {
            try {
                ...
                if (!response.response.ok)
                    throw new Error(`wrapper download failed (${response.response.status})`);
                if (response.bytes.length === 0)
                    throw bootstrapError('wrapper-invalid', 'Gradle Wrapper 下载内容为空');
        ...
        if (errorCode(lastError) === 'checksum-mismatch' || errorCode(lastError) === 'wrapper-invalid')
            throw lastError;
    ...
    update(operation, patch) {
        ...
        if (isTerminal(currentStatus) && patch.status !== currentStatus)
            return;
    ...
    fail(operation, failureCode, message, staging) {
        if (isTerminal(operation.state.snapshot.status))
            return;
        this.update(operation, {
    ...
    publish(snapshot) {
        try {
            this.ctx.emit('minecraft-bootstrap/progress', snapshot);
        }
...
function validateStartRequest(request) {
    ...
    if (row === undefined)
        throw bootstrapError('bad-request', 'start payload must be an object');
    const fields = ['entryId', 'parentDirectory', 'directoryName', 'modName', 'modId', 'packageName'];
    ...
    const parentDirectory = row.parentDirectory;
    const directoryName = row.directoryName;
    const modName = row.modName;
    ...
    if (parentDirectory.length < 1 || parentDirectory.length > 32_000 || !isAbsolute(parentDirectory))
        throw bootstrapError('invalid-path', '保存位置必须是绝对路径');
    if (!isValidDirectoryName(directoryName))
        throw bootstrapError('invalid-directory', '项目目录名无效');
    if (!isValidModName(modName))
        throw bootstrapError('invalid-mod-name', '模组名称无效');
    if (!isValidModId(modId))
        throw bootstrapError('invalid-mod-id', 'modId 必须是小写字母、数字和下划线');
    if (!isValidPackageName(packageName))
        throw bootstrapError('invalid-package', 'Java 包名无效');
}
async function canonicalDirectory(path) {
    ...
    if (info === undefined || !info.isDirectory() || info.isSymbolicLink())
        throw bootstrapError('invalid-path', '保存位置不是可用的真实目录');
    return realpath(path);
}
async function validateTarget(parent, name) {
    const target = resolve(parent, name);
    const rel = relative(parent, target);
    if (rel === '' || rel === '..' || rel.startsWith(`..${sep}`) || isAbsolute(rel))
        throw bootstrapError('path-escape', '项目路径越界');
    await assertNoSymlinkComponents(target);
    const info = await lstat(target).catch(() => undefined);
    if (info !== undefined) {
        if (info.isSymbolicLink())
            throw bootstrapError('target-symlink', '目标目录不能是符号链接');
        if (!info.isDirectory())
            throw bootstrapError('target-exists', '目标路径已存在且不是目录');
        const children = await readdir(target);
        if (children.length > 0)
            throw bootstrapError('target-nonempty', '目标目录必须为空');
...
async function createStaging(path) {
    await assertNoSymlinkComponents(path);
    try {
    ...
    if (!info.isDirectory() || info.isSymbolicLink())
        throw bootstrapError('staging-symlink', 'staging 目录不能是符号链接');
    ...
    if (!stateInfo.isDirectory() || stateInfo.isSymbolicLink())
        throw bootstrapError('staging-symlink', '状态目录不能是符号链接');
}
/** Find a previous failed staging tree, but never resume one the user edited. */
async function findResumableStaging(parent, finalPath, entryId, requestFingerprint, expectedHashes) {
    ...
    for (const item of await readdir(parent, { withFileTypes: true })) {
        ...
        if (!/^[a-f0-9]{8}-(?:[a-f0-9]{4}-){3}[a-f0-9]{12}$/iu.test(suffix))
            continue;
        const info = await lstat(candidate).catch(() => undefined);
        if (info?.isSymbolicLink()) {
            const error = bootstrapError('staging-symlink', 'staging 目录不能是符号链接');
            error.stagingPath = candidate;
            throw error;
        }
        ...
        if (state === undefined)
            throw stagingModified(candidate, 'staging 状态标记缺失或损坏，不能自动续跑');
        if (resolve(state.finalPath) !== resolve(finalPath))
            continue;
        ...
        try {
            await assertStagingTree(candidate, markerMatchesRecipe ? expectedHashes : state.generatedHashes, false);
        }
        catch (error) {
        ...
        if (!markerMatchesRecipe)
            continue;
        if (found !== undefined)
            throw stagingModified(candidate, '存在多个可续跑的 staging，不能安全选择');
        found = { path: candidate, state };
    }
    return found;
}
async function readBootstrapState(staging) {
    try {
        const marker = join(staging, STAGING_MARKER);
        if (await containsSymlink(staging, marker))
            return undefined;
        const markerInfo = await lstat(marker);
        if (!markerInfo.isFile() || markerInfo.size > 256 * 1024)
            return undefined;
        ...
        if (parsed.format !== 2 || typeof parsed.operationId !== 'string' || parsed.operationId.length < 1
            ...
            || Object.entries(hashes).some(([path, hash]) => !isManifestPath(path) || !isSha256(hash)))
            return undefined;
        return parsed;
    }
    catch {
...
function bootstrapRequestFingerprint(request, entry, finalPath) {
...
async function generatedFilesMatch(staging, hashes) {
    for (const [path, expected] of Object.entries(hashes)) {
        try {
            const target = safeJoin(staging, path);
            if (await containsSymlink(staging, target))
                return false;
            const info = await lstat(target);
            if (!info.isFile() || info.isSymbolicLink() || info.size > MAX_GENERATED_FILE_BYTES)
                return false;
...
async function generatedFileMatches(staging, path, expected) {
    try {
        const target = safeJoin(staging, path);
        if (await containsSymlink(staging, target))
            return false;
 ...
 */
async function assertStagingTree(staging, expectedHashes, requireComplete) {
    const allowedInternalFiles = new Set([STAGING_MARKER, '.dsh/bootstrap.log']);
    ...
    const visit = async (directory) => {
        for (const item of await readdir(directory, { withFileTypes: true })) {
            ...
            const info = await lstat(target);
            if (info.isSymbolicLink())
                throw stagingModified(staging, `staging 不能包含符号链接: ${relativePath}`);
            if (info.isDirectory()) {
            ...
            if (!info.isFile())
                throw stagingModified(staging, `staging 包含不支持的文件类型: ${relativePath}`);
            const expected = expectedHashes[relativePath];
            if (expected !== undefined) {
                if (info.size > MAX_GENERATED_FILE_BYTES
                    || sha256(await readFile(target)) !== expected)
                    throw stagingModified(staging, `生成文件已被修改: ${relativePath}`);
                continue;
            }
...
function stagingModified(staging, message) {
    const error = bootstrapError('staging-modified', message);
...
async function containsSymlink(root, target) {
    ...
    if (rootInfo === undefined || rootInfo.isSymbolicLink())
        return true;
...
async function assertNoSymlinkComponents(path) {
    if (!isAbsolute(path))
        throw bootstrapError('invalid-path', '路径必须是绝对路径');
    ...
    for (const part of parts) {
        ...
        if (info === undefined)
            return;
        if (info.isSymbolicLink())
            throw bootstrapError('path-symlink', `路径不能经过符号链接: ${current}`);
    }
}
/** Create an existing-or-new directory below a verified staging root. */
async function ensureDirectoryTree(root, directory) {
...
function safeJoin(root, child) {
    ...
    const rel = relative(root, target);
    if (rel === '..' || rel.startsWith(`..${sep}`) || isAbsolute(rel))
        throw bootstrapError('template-escape', `模板路径越界: ${child}`);
    return target;
}
async function writeState(staging, state) {
    const path = join(staging, STAGING_MARKER);
    if (await containsSymlink(staging, path))
        throw bootstrapError('staging-modified', '状态路径不能包含符号链接');
    await writeFile(path, `${JSON.stringify(state, null, 2)}\n`, { encoding: 'utf8', flag: 'wx' });
}
async function updateState(staging, state) {
    const path = join(staging, STAGING_MARKER);
    if (await containsSymlink(staging, path))
        throw bootstrapError('staging-modified', '状态路径不能包含符号链接');
...
async function appendStateLog(staging, text) {
    const path = join(staging, '.dsh/bootstrap.log');
    if (await containsSymlink(staging, path))
        return;
    await withIoTimeout(ensureDirectoryTree(staging, dirname(path)), LOG_IO_TIMEOUT_MS, '日志目录准备超时');
...
function withIoTimeout(promise, timeoutMs, message) {
...
async function drainWriteChain(chain) {
...
function stagingPathFromError(error) {
    return typeof error === 'object' && error !== null && 'stagingPath' in error
        && typeof error.stagingPath === 'string'
        ? error.stagingPath : undefined;
}
async function verifyWrapper(path, expectedSha256, safeRoot) {
    let bytes;
    try {
        if (await containsSymlink(safeRoot, path))
            throw bootstrapError('wrapper-invalid', 'Gradle Wrapper 路径不能包含符号链接');
        bytes = await readFile(path);
    }
    catch (error) {
        throw bootstrapError('wrapper-invalid', `Gradle Wrapper 无法读取: ${String(error)}`);
    }
    if (bytes.length === 0)
        throw bootstrapError('wrapper-invalid', 'Gradle Wrapper 文件为空');
    if (expectedSha256 !== undefined && sha256(bytes) !== expectedSha256) {
        throw bootstrapError('checksum-mismatch', 'Gradle Wrapper 校验和不匹配');
    }
}
/** Atomically populate the application-owned wrapper cache. */
async function writeCachedWrapper(root, path, bytes) {
    ...
    try {
        ...
        // reading.
        await copyFile(temporary, path, fsConstants.COPYFILE_EXCL).catch((error) => {
            if (error?.code !== 'EEXIST')
                throw error;
        });
    }
...
async function ensureSafeCacheDirectory(cacheDirectory, projectPaths) {
...
async function ensureAbsoluteDirectoryTree(directory) {
...
function parseStartPayload(payload) {
    ...
    for (const field of fields)
        if (typeof row[field] !== 'string')
            throw bootstrapError('bad-request', `${field} must be a string`);
    return {
        entryId: row.entryId, parentDirectory: row.parentDirectory,
        directoryName: row.directoryName, modName: row.modName,
        modId: row.modId, packageName: row.packageName,
    };
}
...
async function runGradleBuild(projectPath, _entry, signal, append, cacheDirectory, java) {
    ...
    if (!hasJar)
        throw bootstrapError('wrapper-missing', 'Gradle Wrapper 文件缺失；不能使用本机 Gradle 回退');
    const args = [
        '-classpath', wrapperJar, 'org.gradle.wrapper.GradleWrapperMain',
        ...await networkGradleArguments(_entry.loader), '--stacktrace', '--no-daemon', 'build',
    ];
    const officialArgs = [
        '-Ddsh.bootstrap.officialOnly=true',
        '-classpath', wrapperJar, 'org.gradle.wrapper.GradleWrapperMain',
        '--stacktrace', '--no-daemon', 'build',
    ];
    ...
    try {
        ...
        if (signal.aborted || Date.now() >= deadline || !isMirrorResolutionFailure(error))
            throw error;
        ...
        try {
            ...
            try {
                await runChild(java, officialArgs, projectPath, signal, append, remaining(), environment);
            }
            catch (officialError) {
                signal.throwIfAborted();
                if (!automaticMinecraftNetwork() || Date.now() >= deadline || !isMirrorResolutionFailure(officialError))
                    throw officialError;
            ...
            if (original !== undefined)
                await writeFile(properties, original, 'utf8');
        }
    }
}
function isMirrorResolutionFailure(error) {
    return isMinecraftNetworkFailure(gradleErrorOutput(error));
}
function gradleErrorOutput(error) {
...
async function replaceDistributionUrl(properties, baseUrl, gradleVersion) {
    ...
    if (!/^distributionUrl=.+$/mu.test(original))
        throw bootstrapError('wrapper-config', 'Gradle Wrapper 配置缺少 distributionUrl');
    if (replacement !== original)
        await writeFile(properties, replacement, 'utf8');
}
async function runChild(executable, args, cwd, signal, append, timeoutMs, overrides = {}) {
    await new Promise((resolvePromise, reject) => {
        ...
        const terminate = (error, drainWrites = true) => {
        ...
        const write = (stream, chunk) => {
            ...
            stream.pause();
            writeChain = writeChain.then(() => append(text)).then(() => { stream.resume(); }, (error) => {
        ...
        const abort = () => { terminate(new DOMException('The operation was aborted', 'AbortError')); };
...
async function fetchResponseBytes(url, signal, fetcher = networkFetch) {
    ...
    try {
        ...
        const response = await awaitWithTimeout(() => fetcher(url, { signal: controller.signal }), CONNECT_TIMEOUT_MS, 'Connection timed out', controller, signal);
        const bytes = await awaitWithTimeout(() => response.arrayBuffer(), READ_TIMEOUT_MS, 'Read timed out', controller, signal);
...
async function awaitWithTimeout(factory, timeoutMs, timeoutMessage, controller, signal) {
    return new Promise((resolvePromise, reject) => {
        let settled = false;
