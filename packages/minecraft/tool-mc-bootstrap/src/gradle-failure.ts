 ...
 */
export function gradleFailureMessage(output: string, code: number | null): string {
  ...
  if (!compilation) {
    ...
    if (/PKIX path building failed/iu.test(output)) return '依赖仓库证书校验失败；检查代理证书配置'
    if (/UnknownHostException/iu.test(output)) return '依赖仓库域名解析失败；检查网络后重试'
    ...
    if (plugin !== null) return `无法解析 Gradle 插件 ${plugin[1]}:${plugin[2]}；检查仓库连接与版本配置`.slice(0, 400)
  }
  const compilerError = output.match(/[^\r\n]*\berror: [^\r\n]+/u)?.[0]?.trim()
  if (compilation && compilerError !== undefined) return `编译失败：${compilerError}`.slice(0, 400)
 ...
 */
export function gradleDistributionDownloadFailed(output: string): boolean {
