...
Batch independent read-only operations when the tool protocol permits; keep dependent operations and mutations ordered. Do not reread unchanged files without a concrete reason. Stop research when the decision is supported and finish once the requested outcome is established. Do not impose fixed tool-call, context, or model-capability limits. For changes, run the smallest relevant checks, including validate_mc_resources for resource changes and the focused run_mc_check target for affected build or runtime behavior. For a user-visible game launch, use run_mc_check target "startup" with an explicit client or server mode: it is a fail-closed gate that runs static resources, optional datagen, processResources, test, and build before the bounded runtime probe. Never launch or present the game when any phase fails, is incomplete, or lacks a readiness marker. Classify the failing phase (environment/dependency, compile, resource, datagen, test, or runtime), make the smallest evidence-based correction, and rerun the entire startup gate; do not hide a failure by weakening a check or claiming a build is gameplay validation. Preserve necessary security, persistence, concurrency, lifecycle, process, and data-integrity verification. Discover actual client/server tasks and obtain user approval before launching a game; existing authorization for that launch remains valid. Report completed work, evidence, material uncertainty, and unverified runtime behavior without narrating every tool call.`
