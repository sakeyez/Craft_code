...
import { findJava } from "./environment.js";
import { runProcess } from "./process.js";
import { networkFetch } from "./network.js";
import { cachedClassSource } from "./sources.js";
 ...
 */
export async function recordApiClasspath(cwd, facts, project, fingerprint, java) {
    if (project.minecraftVersion.status !== 'determined' || project.minecraftVersion.classification !== 'exact')
        return;
    const files = [];
    for (const path of facts.classpath)
        if (path.endsWith('.jar'))
            files.push({ path, sha256: await fileDigest(path) });
    const record = {
        format: 1,
        fingerprint,
 ...
 */
export async function queryMinecraftApi(ctx, cwd, input, signal) {
    ...
    for (const file of data.files) {
        ...
        if ((await fileDigest(file.path)) !== file.sha256)
            throw new Error('依赖 classpath 已变化，请重新构建。');
    }
    ...
    if (!java.executable)
        throw new Error(`缺少 JDK ${data.java} 的 javap；请准备环境。`);
