...
export async function download(
  ...
  if (!response.ok || !response.body) throw new Error(`下载失败：HTTP ${response.status} (${parsed.hostname})`)
  if (Number(response.headers.get('content-length')) > maxBytes) throw new Error('下载超过大小限制。')
 ...
 */
export function zipEntries(bytes: Uint8Array, maxBytes: number = 128 * 1024 * 1024): Record<string, Uint8Array> {
  let total = 0
  let count = 0
 ...
 * @param maxBytes - Maximum accepted complete response or uncompressed size in bytes.
 */
export async function extractZip(bytes: Uint8Array, destination: string, maxBytes?: number): Promise<void> {
...
export async function cachedDownload(
  ...
  if (!/^[a-f\d]{64}$/iu.test(sha256)) throw new Error('发布方未提供有效 SHA-256。')
  const path = join(directory, sha256.toLowerCase())
  const prior = await readFile(path).catch(() => undefined)
  if (prior && hash(prior) === sha256.toLowerCase()) return prior
