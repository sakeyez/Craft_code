...
export class CurseForge {
  ...
  private async request(path: string, signal?: AbortSignal): Promise<unknown> {
    ...
    if (!key) throw new Error('未配置 CurseForge API 密钥。')
  ...
  private filter(project: DetectionResult): string {
    if (
  ...
  async search(
    ...
    const value = z
      ...
      .parse(
        await this.request(
          `mods/search?gameId=432&classId=6&${this.filter(project)}&searchFilter=${encodeURIComponent(query.slice(0, 200))}&pageSize=25`,
          signal,
        ),
      )
  ...
  async file(project: DetectionResult, projectId: number, fileId: number, signal?: AbortSignal): Promise<CurseFile> {
    ...
    if (value.modId !== projectId || value.id !== fileId) throw new Error('CurseForge 文件身份不一致。')
    if (!value.downloadUrl) {
      ...
      const source =
        page.protocol === 'https:' && (page.hostname === 'www.curseforge.com' || page.hostname === 'curseforge.com')
          ? page.href
  ...
  private compatible(project: DetectionResult, file: CurseFile): void {
    if (
      !file.isAvailable ||
      project.minecraftVersion.status !== 'determined' ||
