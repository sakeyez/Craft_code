...
const privateName =
  /^(?:\.env(?:\..*)?|\.npmrc|\.netrc|credentials(?:\..*)?|secrets(?:\..*)?|.*\.(?:pem|key|p12|pfx|jks|keystore))$/iu
 ...
 */
export async function projectInputs(cwd: string, signal?: AbortSignal): Promise<ProjectInputs> {
  ...
  const visit = async (directory: string): Promise<void> => {
    ...
    for (const row of rows.sort((a, b) => a.name.localeCompare(b.name))) {
      ...
      if (excluded.has(row.name) || privateName.test(row.name)) continue
      ...
      if (path === '.dsh') {
        for (const owned of ['.dsh/dependencies.json', '.dsh/dependencies.gradle']) {
          ...
          if (info?.isFile()) files[owned] = await fileDigest(file)
        }
        continue
      }
      if (row.isSymbolicLink()) throw new Error(`项目输入包含符号链接，无法完整验收：${path}`)
      if (row.isDirectory()) await visit(path)
      else if (row.isFile()) {
      ...
      } else throw new Error(`项目输入不是普通文件：${path}`)
    }
  }
  await visit('')
