...
import { z } from 'zod'
import { inspectJar } from './dependencies.ts'
import { zipEntries } from './download.ts'
...
export async function acceptArtifact(
  ...
  const published = facts.artifacts.filter(
    row =>
      !/sources|javadoc|dev|test/iu.test(row.classifier) &&
  ...
  if (!selected) throw new Error('没有候选 JAR。')
  const file = await projectPath(cwd, selected.path)
  if ((await stat(file)).size > 256 * 1024 ** 2) throw new Error('产物超过 256 MiB 验收限制。')
  const bytes = await readFile(file)
  const metadata = inspectJar(bytes)
  if (
    (project.loader !== 'fabric' && project.loader !== 'neoforge') ||
  ...
  if (!ids.includes(metadata.modId)) throw new Error(`产物模组 ID ${metadata.modId} 与项目不一致。`)
  ...
  const coordinates = facts.modules.filter(row =>
  ...
  for (const resource of facts.resources ?? [])
    if (!entries[resource]) throw new Error(`产物缺少 Gradle 输出资源：${resource}`)
  const nestedIds: string[] = []
  const nestedVersions: Record<string, string> = {}
  if (project.loader === 'fabric') {
    const fabric = z
      .object({
        ...
        entrypoints: z
          .record(
            z.string(),
            z.array(z.union([z.string(), z.object({ value: z.string(), adapter: z.string().optional() })])),
          )
          .default({}),
      })
    ...
    for (const item of fabric.jars) {
      ...
      if (!nested) throw new Error(`产物缺少嵌套依赖：${item.file}`)
    ...
    for (const mixin of fabric.mixins) {
      ...
      if (!entries[path]) throw new Error(`产物缺少 Mixin 配置：${path}`)
    }
    for (const item of Object.values(fabric.entrypoints).flat()) {
      ...
      if (!entries[entry.replaceAll('.', '/') + '.class']) throw new Error(`产物缺少入口类：${entry}`)
    }
  }
  if (project.loader === 'neoforge' && entries['META-INF/jarjar/metadata.json']) {
    ...
    for (const item of nested.jars) {
      ...
      if (!bytes) throw new Error(`产物缺少嵌套依赖：${item.path}`)
 ...
 */
export function buildInspectionScript(task: string, destination: string): string {
      ...
      ['compileClasspath', 'runtimeClasspath'].each { name ->
        ...
        if (conf != null && conf.canBeResolved) {
          conf.resolvedConfiguration.resolvedArtifacts.each { artifact ->
            ...
            // NeoGradle publishes its transformed runtime through an internal repository group.
            if (project.plugins.hasPlugin('net.neoforged.gradle.userdev') && id.group == 'ng_dummy_ng.net.neoforged' && id.name == 'neoforge') {
              modules.add([group: 'net.neoforged', name: 'neoforge', version: id.version])
            }
          }
