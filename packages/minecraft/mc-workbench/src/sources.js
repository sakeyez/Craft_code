...
import { cachedDownload, extractZip, zipEntries } from "./download.js";
import { manifestSchema } from "./contracts.js";
import { z } from 'zod';
import { cacheRoot, ensureJava, javaEnv } from "./environment.js";
import { hash, listFiles, projectPath, readText, searchFiles } from "./files.js";
 ...
 */
export async function cachedClassSource(cwd, jarHashes, symbol) {
    ...
    for (const dependency of manifestSchema.parse(JSON.parse(manifest)).dependencies) {
        if (!jarHashes.has(dependency.sha256))
            continue;
        ...
        if (hash(bytes) !== entry.sha256)
            throw new Error('反编译缓存校验失败，请重新准备源码。');
        return {
...
export class MinecraftSources {
     ...
     */
    async start(cwd, dependency, sourceArchive) {
        const bytes = await readFile(await projectPath(cwd, dependency.file));
        if (hash(bytes) !== dependency.sha256)
            throw new Error('依赖 JAR 已变化，请重新导入。');
        const id = workbenchIdSchema.parse(randomUUID());
        const controller = new AbortController();
        const directory = join(cacheRoot(), 'sources', id);
        const sourcePath = sourceArchive ?? (dependency.sourcesFile ? await projectPath(cwd, dependency.sourcesFile) : undefined);
        const archiveBytes = sourcePath ? await readFile(sourcePath) : undefined;
        if (archiveBytes && archiveBytes.length > 128 * 1024 * 1024)
            throw new Error('源码归档超过 128 MiB 限制。');
        ...
        const operation = { cwd, snapshot, directory, controller, done: Promise.resolve() };
        ...
        operation.done = (async () => {
            ...
            if (archiveBytes)
                await extractZip(archiveBytes, directory);
            else {
                const java = await ensureJava(this.ctx, 21, controller.signal, async () => { });
                if (!java.executable)
                    throw new Error('Java 尚未就绪。');
                const engine = await cachedDownload(join(cacheRoot(), 'downloads'), `https://repo.maven.apache.org/maven2/org/vineflower/vineflower/${ENGINE_VERSION}/vineflower-${ENGINE_VERSION}.jar`, ENGINE_SHA, controller.signal);
                ...
                if (result.exitCode !== 0)
                    throw new Error(`反编译失败：${result.text.slice(-2000)}`);
            }
            controller.signal.throwIfAborted();
            const files = [];
    ...
    operation(cwd, id) {
        ...
        if (!operation || operation.cwd !== cwd)
            throw new Error('源码任务不属于当前项目。');
        return operation;
    }
    /**
     ...
     */
    async files(cwd, id, path) {
        const operation = this.operation(cwd, id);
        if (operation.snapshot.status !== 'ready')
            throw new Error('源码尚未就绪。');
