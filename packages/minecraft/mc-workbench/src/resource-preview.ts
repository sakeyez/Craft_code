...
import { projectPath, hash, searchFiles } from './files.ts'
...
async function archiveEntry(file: string, name: string): Promise<Buffer | undefined> {
...
export async function previewResource(
  ...
  if ((await stat(path)).size > 4 * 1024 ** 2) throw new Error('预览文件超过 4 MiB。')
  ...
  if (!namespace || !resourceName) throw new Error('资源路径缺少命名空间或名称。')
  let retainedBytes = bytes.length
  let vanillaVerified = false
  const resource = async (id: string, kind: 'models' | 'textures'): Promise<Buffer | undefined> => {
    if (!/^(?:[a-z\d_.-]+:)?[a-z\d_./-]+$/u.test(id) || id.includes('..')) throw new Error('资源引用无效。')
    ...
    for (const root of project.resourceRoots) {
      ...
      if (!info) continue
      if (!info.isFile() || info.size > 4 * 1024 ** 2) throw new Error('引用资源超过预览限制。')
  ...
  if (request.path.endsWith('.json')) {
    ...
    const resolveModel = async (
      text: string,
      depth: number,
    ): Promise<z.infer<typeof modelSchema> & { generated: boolean }> => {
      if (depth > 16) throw new Error('模型继承超过 16 层。')
      const own = modelSchema.parse(JSON.parse(text))
      if (own.loader || own.overrides) result.unsupported.push('自定义加载器或动态覆盖不支持预览。')
      if (!own.parent) return { ...own, generated: false }
      if (
        ['item/generated', 'minecraft:item/generated', 'builtin/generated', 'minecraft:builtin/generated'].includes(
          own.parent,
        )
      )
        return { ...own, generated: true }
      if (seen.has(own.parent)) throw new Error('模型继承包含循环。')
      seen.add(own.parent)
    ...
    for (const [key, value] of Object.entries(model.textures)) {
      ...
      if (retainedBytes > 8 * 1024 ** 2) throw new Error('预览引用超过 8 MiB。')
    ...
    for (const part of result.elements)
      for (const face of Object.values(part.faces)) {
        ...
        } else if (!result.images[face.texture.slice(1)]) result.missing.push(face.texture)
      }
    result.kind =
  ...
  if (search.truncated) result.unsupported.push('引用结果不完整，请缩小项目范围。')
