...
import { z } from 'zod';
import { download, fetchJson, zipEntries } from "./download.js";
import { hash, projectPath, readText } from "./files.js";
...
import { matchesDependencyVersion } from "./dependency-version.js";
 ...
 */
export function inspectJar(bytes) {
    const entries = zipEntries(bytes);
    const decode = (name) => entries[name] ? new TextDecoder().decode(entries[name]) : undefined;
    const fabric = decode('fabric.mod.json');
    if (fabric) {
        const value = JSON.parse(fabric);
        const requirements = {};
        if (value.depends && typeof value.depends === 'object')
            for (const [id, constraint] of Object.entries(value.depends)) {
                if (typeof constraint === 'string')
                    ...
                    throw new Error(`模组依赖 ${id} 的版本约束无效。`);
            }
        return {
    ...
    if (neo) {
        ...
        const requirementSides = {};
        const deps = value.dependencies && typeof value.dependencies === 'object' && !Array.isArray(value.dependencies)
 ...
 */
export function inspectJarTree(bytes) {
    let remaining = 256 * 1024 ** 2;
    let count = 0;
    const visit = (archive, depth) => {
        if (depth > 8 || ++count > 200)
            throw new Error('嵌套依赖超过验收限制。');
...
export class MinecraftDependencies {
     ...
     */
    async read(cwd) {
        if (this.busy.has(cwd))
            throw new Error('依赖事务正在提交，请稍后读取。');
        this.busy.add(cwd);
        try {
    ...
    async recover(cwd) {
        ...
        for (const change of [...changes].reverse()) {
            if (![MANIFEST, GRADLE, 'build.gradle', 'build.gradle.kts'].includes(change.path) &&
            ...
            if (change.before === null)
                ...
                await writeFile(file, change.before);
        }
    ...
    async search(project, query, signal) {
        ...
        const data = (await fetchJson(`https://api.modrinth.com/v2/search?query=${encodeURIComponent(query.slice(0, 200))}&facets=${encodeURIComponent(facets)}&limit=25`, signal));
        if (!Array.isArray(data.hits))
            throw new Error('Modrinth 搜索结果无效。');
    ...
    async releases(project, id, signal) {
        const data = await fetchJson(`https://api.modrinth.com/v2/project/${encodeURIComponent(id)}/version?game_versions=${encodeURIComponent(JSON.stringify([version(project)]))}&loaders=${encodeURIComponent(JSON.stringify([project.loader]))}`, signal);
        if (!Array.isArray(data))
            throw new Error('Modrinth 版本列表无效。');
     ...
     */
    async versions(project, id, signal) {
     ...
     */
    async preview(cwd, project, input, signal) {
        version(project);
        ...
        for (const entry of next) {
            ...
            for (const [id, ranges] of Object.entries(descriptor.requirements)) {
                ...
                if (installed && !entry.dependencies.includes(installed.id))
                    entry.dependencies.push(installed.id);
            }
            entry.warnings = [...new Set(entry.warnings)];
        }
        const ids = next.flatMap(row => (row.modId ? [row.modId] : []));
        if (new Set(ids).size !== ids.length)
            throw new Error('检测到相同 mod ID 的多个文件，请先解决冲突。');
        const checked = new Set();
        const acyclic = (id, ancestors) => {
            if (ancestors.has(id))
                throw new Error('依赖关系包含循环。');
            if (checked.has(id))
                return;
    ...
    async resolve(cwd, project, source, role, automatic, entries, artifacts, publications, signal) {
        signal?.throwIfAborted();
        if (entries.length >= 100)
            throw new Error('依赖数量超过 100，请缩小导入范围。');
        let bytes;
        let id;
        let name;
        let resolvedVersion;
        ...
        if (source.kind === 'modrinth') {
            releaseInfo = release(await fetchJson(`https://api.modrinth.com/v2/version/${encodeURIComponent(source.versionId)}`, signal));
            if (releaseInfo.project_id !== source.projectId ||
            ...
            if (!artifact || !artifact.filename.endsWith('.jar'))
                throw new Error('发布版本没有可安装的 JAR。');
            ...
            if (!match || source.coordinate.includes('..'))
                throw new Error('Maven 坐标格式应为 group:artifact:version。');
            ...
            if (!group || !artifact || !number)
                throw new Error('Maven 坐标不完整。');
            ...
            const existing = entries.find(row => row.id === id);
            if (existing) {
                if (existing.version !== resolvedVersion)
                    throw new Error('Maven 依赖版本冲突。');
                return existing;
            }
        ...
        if (descriptor.modId && !/^[a-z][a-z\d_-]{1,63}$/u.test(descriptor.modId))
            throw new Error('JAR 中的 mod ID 无效。');
        if (descriptor.loader && descriptor.loader !== project.loader)
            throw new Error('JAR 加载器与项目不一致。');
        ...
        for (const required of curseInfo?.dependencies ?? []) {
            ...
            if (!this.curseforge)
                throw new Error('未配置 CurseForge 服务。');
            const candidate = (await this.curseforge.files(project, required.modId, signal))[0];
            if (!candidate)
                throw new Error(`${name} 的必需前置没有兼容版本。`);
            const child = await this.resolve(cwd, project, { kind: 'curseforge', projectId: required.modId, fileId: candidate.id }, 'required', true, entries, artifacts, publications, signal);
        ...
        for (const dependency of releaseInfo?.dependencies ?? []) {
            ...
            if (dependency.version_id)
                target = release(await fetchJson(`https://api.modrinth.com/v2/version/${encodeURIComponent(dependency.version_id)}`, signal));
            ...
            if (!target)
                throw new Error(`${entry.name} 的必需前置没有兼容版本。`);
            const child = await this.resolve(cwd, project, { kind: 'modrinth', projectId: target.project_id, versionId: target.id }, 'required', true, entries, artifacts, publications, signal);
            entry.dependencies.push(child.id);
        }
        return entry;
    }
    async changes(cwd, project, prior, next) {
        ...
        const expected = project.loader === 'fabric'
        ...
        for (const row of next.filter(row => !row.automatic)) {
            ...
            if (row.role !== 'test')
                add(row, compilation);
            if (row.role !== 'optional' || row.enabled)
                add(row, runtime);
        }
        for (const row of next) {
            const compile = compilation.has(row.id);
            const isRequired = required.has(row.id);
            const configurations = project.loader === 'fabric'
                ? [
            ...
            for (const config of configurations)
                lines.push(`    ${config} files(rootProject.file('${row.file}'))`);
        }
        lines.push('}', '');
        ...
        const metadataPath = project.inspected.metadataFiles.find(path => project.loader === 'fabric' ? path.endsWith('fabric.mod.json') : /(?:neoforge\.)?mods\.toml$/u.test(path));
        if (!metadataPath)
            throw new Error('找不到当前模组的依赖声明文件。');
        const metadata = await readText(cwd, metadataPath);
        const ownId = project.modIdCandidates.find(row => row.confidence === 'high')?.id;
        if (!ownId)
            throw new Error('无法确定当前项目 mod ID。');
        let after;
        if (project.loader === 'fabric') {
            ...
            for (const key of ['depends', 'suggests']) {
                ...
                const values = Object.fromEntries(Object.entries((data[key] ?? {})).filter(([id]) => !previousIds.has(id)));
                for (const row of next)
            ...
            const all = (data.dependencies ?? {});
            for (const row of next)
                if (row.modId &&
                    !prior.some(old => old.modId === row.modId) &&
                    Array.isArray(all[ownId]) &&
                    all[ownId].some(old => old.modId === row.modId))
                    throw new Error(`${row.modId} 已有手写依赖声明，不能自动覆盖。`);
            const previousIds = new Set(prior.flatMap(row => (row.modId ? [row.modId] : [])));
            const rows = (Array.isArray(all[ownId]) ? all[ownId] : []).filter(row => !previousIds.has(String(row.modId)));
            for (const row of next)
    ...
    async commit(cwd, id) {
        ...
        if (!pending || pending.cwd !== cwd)
            throw new Error('依赖预览已过期，请重新预览。');
        if (this.busy.has(cwd))
            throw new Error('此项目正在修改依赖。');
        this.busy.add(cwd);
        const written = [];
        const read = async (path) => readFile(await projectPath(cwd, path, true), 'utf8').catch((error) => {
            if (error instanceof Error && 'code' in error && error.code === 'ENOENT')
                return null;
            throw error;
        });
        try {
            ...
            for (const change of pending.plan.changes)
                if ((await read(change.path)) !== change.before)
                    throw new Error(`${change.path} 已变化，请重新预览。`);
            ...
            for (const change of pending.plan.changes) {
                if ((await read(change.path)) !== change.before)
                    throw new Error(`${change.path} 已变化，正在回滚。`);
            ...
            await unlink(journal);
            this.plans.delete(id);
            const retainedFiles = new Set(pending.plan.dependencies.flatMap(row => [row.file, ...(row.sourcesFile ? [row.sourcesFile] : [])]));
            const priorManifest = pending.plan.changes.find(change => change.path === MANIFEST)?.before;
            if (priorManifest)
                for (const row of JSON.parse(priorManifest).dependencies)
            ...
            for (const change of written.reverse()) {
                if ((await read(change.path)) !== change.after)
                    continue;
                const target = await projectPath(cwd, change.path);
                if (change.before === null)
                    await unlink(target);
                else
                    await writeFile(target, change.before);
            }
            throw error;
        }
        finally {
