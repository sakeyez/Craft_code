...
import { installerDispatcher, networkFetch, minecraftDownloadSources, networkJavaEnvironment } from './network.ts'
...
interface InstallTask<T> {
  ...
  total: number
  cancel(): Promise<void>
  startAndWait(context?: { onUpdate?: () => void }): Promise<T>
}
...
export async function prepareInstance(
  ...
  const run = async <T>(task: InstallTask<T>): Promise<T> => {
    ...
    try {
      ...
      if (error instanceof AggregateError) {
        ...
        throw Object.assign(
          new Error(
            `安装失败：${[...new Set(causes)].slice(0, 3).join('；')}${causes.length > 3 ? `；另有 ${causes.length - 3} 项失败` : ''}`,
          ),
          {
            code:
              disk?.code ??
              (error.errors.some((value: unknown) => value instanceof Error && value.name === 'ChecksumNotMatchError')
                ? 'CHECKSUM'
                : 'NETWORK'),
          },
        )
  ...
  try {
    ...
    const fetcher = (url: string, init?: RequestInit): Promise<Response> => networkFetch(url, { ...init, signal })
    const manifest = z
    ...
    if (!metadata) throw new Error(`官方版本列表没有 Minecraft ${artifact.minecraft}。`)
    const options: Options = {
      // XMCL pins an older Undici 7 declaration; both consume the stable dispatch protocol.
      side,
      dispatcher: (await installerDispatcher()) as unknown as NonNullable<Options['dispatcher']>,
      fetch: fetcher as typeof fetch,
      json: meta => minecraftDownloadSources(meta.url),
      client: (version) => {
        if (!version.downloads.client) throw new Error('官方元数据缺少客户端。')
        return minecraftDownloadSources(version.downloads.client.url)
      },
      server: (version) => {
        if (!version.downloads.server) throw new Error('官方元数据缺少服务端。')
    ...
    if (artifact.loader === 'fabric') {
      ...
      versionId = await run(
        installNeoForgedTask('neoforge', artifact.loaderVersion, folder, {
          ...options,
          java,
          spawn: (command, args = []) => {
    ...
    if (side === 'client') {
      ...
      const version: ResolvedVersion = Version.resolve(folder, [Version.normalizeVersionJson(text, root)])
      ...
      const jvm = version.arguments.jvm.map((arg) => {
        if (typeof arg !== 'string') throw new Error('服务端 JVM 启动规则尚未支持。')
        return arg
          .replace(/\$\{([^}]+)\}/gu, (_, key: string) => {
            const replacement = replacements[key]
            if (replacement === undefined) throw new Error(`无法解析服务端参数：${key}`)
            return replacement
          })
          .replace(/(^|[;:=])libraries[\\/]/gu, `$1${folder.libraries.replaceAll('\\', '/')}/`)
      })
      const game = version.arguments.game.map((arg) => {
        if (typeof arg !== 'string') throw new Error('服务端游戏启动规则尚未支持。')
        return arg
      })
      argv = [
        java,
      ...
      const priorProperties = await readFile(properties, 'utf8').catch((error: unknown) => {
      ...
      await writeFile(
        properties,
        [...lines, ...Object.entries(managed).map(([key, value]) => `${key}=${value}`), ''].join('\n'),
      )
    ...
    signal.throwIfAborted()
    if (!argv[0] || resolve(argv[0]) !== resolve(java)) throw new Error('安装库未返回预期 Java 启动命令。')
