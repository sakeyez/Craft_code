...
import { z } from 'zod'
import { matchesDependencyVersion as matches } from './dependency-version.ts'
import { projectPath, hash } from './files.ts'
import { fileDigest } from './transfer.ts'
import { inspectJarTree } from './dependencies.ts'
...
async function optionalRead(file: string): Promise<Buffer | undefined> {
 ...
 */
export async function recoverInstanceMods(directory: string): Promise<void> {
  ...
  for (const change of value.changes) {
    ...
    if (change.before) {
      ...
      if ((await fileDigest(backup)) !== change.before) throw new Error('实例事务备份损坏。')
 ...
 */
export async function syncInstanceMods(
  cwd: string,
  directory: string,
  ...
  const walk = (id: string, ancestors: Set<string>): void => {
    if (ancestors.has(id)) throw new Error('测试依赖包含循环。')
    const row = dependencies.find(dep => dep.id === id)
    if (!row) throw new Error(`缺少已锁定依赖：${id}`)
    for (const child of row.dependencies) {
      retained.add(child)
      walk(child, new Set([...ancestors, id]))
    }
  }
  for (const id of [...retained]) walk(id, new Set())
  const artifacts = [
  ...
  for (const row of artifacts) {
    ...
    if (hash(bytes) !== row.sha256) throw new Error(`测试文件校验失败：${row.file}`)
    const tree = inspectJarTree(bytes)
    const descriptor = tree[0]
    if (!descriptor) continue
    if (descriptor.side && descriptor.side !== side) continue
  ...
  if (!items.some(row => row.sha256 === artifact.sha256)) throw new Error('成品模组不支持所选运行侧。')
  ...
  for (const descriptor of descriptors) {
    ...
    if (identities.has(id)) throw new Error(`测试实例存在重复 mod ID：${id}`)
    identities.set(id, version)
  }
  for (const descriptor of descriptors) {
    for (const incompatible of descriptor.incompatible ?? []) {
      if (descriptor.requirementSides?.[incompatible] && descriptor.requirementSides[incompatible] !== side) continue
    ...
    for (const [id, ranges] of Object.entries(descriptor.requirements)) {
      if (descriptor.requirementSides?.[id] && descriptor.requirementSides[id] !== side) continue
      const actual = identities.get(id)
      if (!actual || !matches(actual, ranges)) throw new Error(`测试实例缺少兼容依赖：${id} ${ranges.join(' 或 ')}`)
    }
  }
  const statePath = await projectPath(directory, 'mods-state.json', true)
  const prior = await optionalRead(statePath)
  const before = prior ? stateSchema.parse(JSON.parse(prior.toString())) : { format: 1 as const, files: {} }
  for (const [name, expected] of Object.entries(before.files)) {
    ...
    if (!content || hash(content) !== expected) throw new Error(`实例文件被外部修改或缺失：${name}`)
  }
  const after = {
    format: 1 as const,
    files: Object.fromEntries(
      items.map(row => [`${row.descriptor.modId}-${row.sha256.slice(0, 12)}.jar`, row.sha256]),
    ),
  }
  ...
  for (const change of changes) {
    ...
    if ((content ? hash(content) : null) !== change.before) throw new Error(`实例文件被外部修改：${change.name}`)
    ...
    if (change.after) {
      ...
      if (!source) throw new Error('实例事务缺少已校验文件。')
