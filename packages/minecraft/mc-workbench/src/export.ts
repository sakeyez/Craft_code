 ...
 */
export async function exportMod(cwd: string, runs: RunSnapshot[]): Promise<{ path: string; artifacts: string[] }> {
  const inputs = await projectInputs(cwd)
  const run = runs.find(row => row.artifact?.inputFingerprint === inputs.fingerprint)
  const artifact = run?.artifact
  if (!artifact) throw new Error('没有对应当前源码的验收产物，请以成品模式构建。')
  ...
  if ((await fileDigest(file)) !== artifact.sha256) throw new Error('产物已变化，请重新构建验收。')
  const id = randomUUID()
  ...
  if (
    (await fileDigest(join(staging, name))) !== artifact.sha256 ||
  ...
  await writeFile(
    ...
    JSON.stringify({ format: 1, artifact, build: 'passed', runtime, gameplay: 'unverified' }, null, 2) + '\n',
  )
