 ...
 */
export async function projectPath(root, path, missing = false) {
    ...
    for (const part of parts) {
        ...
        if (stat?.isSymbolicLink())
            throw new Error(`不支持通过符号链接访问：${path}`);
        if (!stat && missing)
            break;
    }
    return target;
 ...
 */
export async function readText(root, path, readonly = false) {
    const file = await projectPath(root, path);
    const stat = await lstat(file);
    if (!stat.isFile() || stat.size > MAX_TEXT_BYTES)
        throw new Error('此文件过大或不是普通文件，请在外部编辑器中打开。');
    const bytes = await readFile(file);
    if (bytes.includes(0))
        throw new Error('二进制文件不能作为文本编辑。');
    let text;
    try {
...
export async function saveText(root, path, text, revision) {
    ...
    try {
        ...
        if ((await readText(root, path)).revision !== revision)
            throw new Error('文件已变化，未覆盖磁盘内容。');
...
export async function listFiles(root, path = '') {
    ...
    if (rows.length > 5000)
        throw new Error('目录条目过多，请打开更具体的目录。');
    return rows
...
export async function searchFiles(root, query) {
    if (!query.trim() || query.length > 256)
        throw new Error('请输入 1–256 字符的搜索内容。');
    const pending = [''];
    const hits = [];
    let files = 0;
