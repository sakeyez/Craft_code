...
import { projectPath, hash } from './files.ts'
...
export interface RestorePreview {
  id: string
  fingerprint: string
  changes: (z.infer<typeof change> & { beforeText?: string; afterText?: string })[]
}
...
function protectedPath(path: string): void {
  if (
    ...
    /(?:^|[\\/])(?:\.env(?:\..*)?|\.npmrc|\.netrc|credentials(?:\..*)?|secrets(?:\..*)?|.*\.(?:pem|key|p12|pfx|jks|keystore))$/iu.test(
      path,
    ) ||
...
export class MinecraftCheckpoints {
  private static readonly active = new Map<string, Promise<unknown>>()
  private static readonly edits = new Map<string, Promise<unknown>>()
  ...
  async mutate<T>(cwd: string, work: () => Promise<T>): Promise<T> {
    ...
    try {
      ...
      if (MinecraftCheckpoints.edits.get(cwd) === result) MinecraftCheckpoints.edits.delete(cwd)
    }
  }
  ...
  private async exclusive<T>(cwd: string, work: () => Promise<T>): Promise<T> {
    ...
    try {
      ...
      if (MinecraftCheckpoints.active.get(cwd) === result) MinecraftCheckpoints.active.delete(cwd)
    }
  }
  /**
  ...
  async list(cwd: string): Promise<ProjectCheckpoint[]> {
    ...
    const rows: ProjectCheckpoint[] = []
    for (const name of names.filter(name => /^[a-f\d-]{36}\.json$/u.test(name))) {
      const row = manifest.parse(
        JSON.parse((await readFile(await projectPath(cwd, `${base}/records/${name}`))).toString()),
      )
      if (`${row.id}.json` !== name) throw new Error('恢复点标识不一致。')
      rows.push(row)
    }
  ...
  private async capture(
    ...
    for (const [path, sha] of Object.entries(inputs.files)) {
      ...
      if ((await fileDigest(target).catch(() => '')) === sha) continue
      const temporary = target + '.' + randomUUID() + '.tmp'
      try {
        await copyFile(await projectPath(cwd, path), temporary)
        if ((await fileDigest(temporary)) !== sha) throw new Error(`备份期间文件发生变化：${path}`)
    ...
    const record: ProjectCheckpoint = {
      format: 1,
      id: randomUUID(),
  ...
  async preview(cwd: string, id: string): Promise<RestorePreview> {
    return this.exclusive(cwd, async () => {
      ...
      if (!record) throw new Error('恢复点不存在。')
      ...
      const previews: RestorePreview['changes'] = []
      for (const row of changes) {
        ...
        const after = row.after
          ...
          : Buffer.alloc(0)
        if (row.after && hash(after) !== row.after) throw new Error('恢复点内容校验失败。')
        const size = before.length + after.length
        if (
          size <= Math.min(65536, remaining) &&
          !before.includes(0) &&
          !after.includes(0) &&
  ...
  restore(cwd: string, id: string, fingerprint: string): Promise<void> {
    return this.mutate(cwd, () =>
      this.exclusive(cwd, async () => {
        await this.recover(cwd)
        const record = (await this.list(cwd)).find(row => row.id === id)
        if (!record) throw new Error('恢复点不存在。')
        const current = await projectInputs(cwd)
        if (current.fingerprint !== fingerprint) throw new Error('项目在预览后已变化，请重新查看恢复差异。')
        ...
        try {
          for (const row of changes) {
            ...
            if ((bytes ? hash(bytes) : null) !== row.before) throw new Error(`恢复期间文件发生变化：${row.path}`)
  ...
  private async replace(cwd: string, path: string, sha: string | null): Promise<void> {
    protectedPath(path)
    ...
    if ((await fileDigest(object)) !== sha) throw new Error('恢复点内容校验失败。')
  ...
  private async recover(cwd: string): Promise<void> {
    ...
    for (const row of journal.changes) {
      ...
      if (current !== row.before && current !== row.after) throw new Error(`恢复事务遇到外部修改：${row.path}`)
    }
    for (const row of journal.changes) await this.replace(cwd, row.path, row.before)
    await unlink(file)
  }
  /**
