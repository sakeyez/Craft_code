...
const scope = new AsyncLocalStorage<NetworkOperation>()
 ...
 */
export function minecraftDownloadSources(input: string): string[] {
  ...
  if (url.hostname === 'maven.fabricmc.net') mirror = 'https://bmclapi2.bangbang93.com/maven' + url.pathname
  if (url.hostname === 'libraries.minecraft.net') mirror = 'https://bmclapi2.bangbang93.com/maven' + url.pathname
  ...
  if (
    ['piston-meta.mojang.com', 'piston-data.mojang.com', 'launchermeta.mojang.com', 'launcher.mojang.com'].includes(
      url.hostname,
    )
  )
    mirror = 'https://bmclapi2.bangbang93.com' + url.pathname
  if (
    ['services.gradle.org', 'downloads.gradle.org'].includes(url.hostname) &&
...
async function desktopProxy(url: string): Promise<string | undefined> {
  if (!process.send || process.env.DSH_SUPERVISOR_IPC !== '1') return undefined
  const id = randomUUID()
  return new Promise((resolve, reject) => {
    ...
    const listener = (message: unknown): void => {
      ...
      } else finish('proxy' in message && typeof message.proxy === 'string' ? message.proxy : undefined)
    }
...
export class NetworkOperation {
  constructor(
    ...
    private readonly inheritedProxy?: string,
    private readonly agents = new Map<string, Dispatcher>(),
    private readonly successful = new Map<string, { source: string; proxy?: string; until: number }>(),
  ) {}

  private async proxy(url: string): Promise<string | undefined> {
    if (this.settings.mode === 'direct') return undefined
    if (this.settings.mode === 'proxy') return validateProxy(this.settings.proxyUrl)
    const inherited = this.inheritedProxy
    if (inherited) return validateProxy(inherited)
    const system = await desktopProxy(url)
    if (!system || system === 'DIRECT') return undefined
   ...
   */
  async installerDispatcher(): Promise<Dispatcher> {
    const proxy = await this.proxy('https://piston-meta.mojang.com/mc/game/version_manifest_v2.json')
    if (!proxy) return direct.compose(interceptors.redirect({ maxRedirections: 5 }))
  ...
  async fetch(
    ...
    sha256?: string,
    sink?: (response: Response) => Promise<void>,
    headersForAttempt?: () => HeadersInit,
  ): Promise<Response> {
    const url = new URL(input)
    ...
    try {
      ...
      if (this.settings.mode !== 'auto') throw error
      ...
      this.settings.mode === 'proxy'
        ...
        : [
          ...sources
            ...
            .map(source => ({ source, proxy: undefined as string | undefined })),
    ...
    if (preferred && preferred.until > Date.now())
      routes.sort(
        (a, b) =>
          Number(b.source === preferred.source && b.proxy === preferred.proxy) -
          Number(a.source === preferred.source && a.proxy === preferred.proxy),
      )
    const attempts: string[] = proxyError ? [proxyError] : []
    let failureCode = 'NETWORK'
    for (const route of routes) {
      let dispatcher: Dispatcher = direct
      if (route.proxy) {
        dispatcher =
      ...
      for (let attempt = 0; attempt < 3; attempt++) {
        ...
        try {
          ...
          if (sink && (response.ok || response.status === 416)) {
            await sink(response)
            if (this.successful.size >= 64) this.successful.clear()
          ...
          if (response.ok) {
            ...
            try {
              while (reader) {
                ...
                if (size > maxBytes) throw new Error('下载超过大小限制。')
            ...
            if (response.headers.get('content-type')?.includes('application/json')) JSON.parse(bytes.toString('utf8'))
          ...
          if (response.status === 429) {
            ...
            if (duration > 30_000) break
            if (Number.isFinite(duration)) retryDelay = Math.max(retryDelay, duration)
          }
          if (response.status !== 429 && response.status < 500) break
        } catch (error) {
          signal.throwIfAborted()
          if (error instanceof Error && 'code' in error) {
            if (['ENOSPC', 'EACCES', 'EPERM', 'EROFS', 'EIO', 'LIMIT'].includes(String(error.code))) throw error
        ...
        if (attempt < 2) await delay(retryDelay, undefined, { signal })
      }
    }
    throw Object.assign(
      new Error(`下载失败：${[...new Set(attempts)].join('；')}。请打开 Minecraft 下载与构建设置检测连接或调整代理。`),
      { code: failureCode },
    )
  }
   ...
   */
  async javaEnvironment(directFallback = false): Promise<NodeJS.ProcessEnv> {
    const proxy =
      directFallback && this.settings.mode === 'auto'
        ? undefined
    ...
    if (proxy) {
      ...
      if (url.protocol === 'https:') throw new Error('Java 构建不支持 TLS 代理连接，请使用 VPN 的 HTTP／混合代理端口。')
    ...
    } else options.push('-Dhttp.proxyHost=', '-Dhttps.proxyHost=', '-DsocksProxyHost=')
...
export class MinecraftNetwork {
  ...
  constructor(private readonly ctx: Context) {
    ctx.effect(
      () => () => Promise.allSettled([...this.agents.values()].map(agent => agent.destroy())).then(() => {}),
  ...
  async save(input: unknown): Promise<NetworkSettings> {
    ...
    if (!settings) throw new Error('宿主尚未提供设置存储。')
   ...
   */
  async check(signal: AbortSignal): Promise<{ name: string; ok: boolean; message: string }[]> {
    return this.run(async () => {
      const java = await networkJavaEnvironment().then(
        () => ({ name: 'Java 代理配置', ok: true, message: '可用于 Gradle 子进程' }),
        (error: unknown) => ({
          name: 'Java 代理配置',
          ok: false,
 ...
 */
export function networkFetch(url: string, init?: RequestInit, maxBytes?: number, sha256?: string): Promise<Response> {
 ...
 * @returns Shared proxy-aware dispatcher with bounded connections.
 */
export function installerDispatcher(): Promise<Dispatcher> {
 ...
 */
export async function networkTransfer(
  url: string,
  init: RequestInit,
  maxBytes: number,
  sink: (response: Response) => Promise<void>,
  ...
  await (scope.getStore() ?? new NetworkOperation({ mode: 'auto', proxyUrl: '' })).fetch(
    url,
    init,
    maxBytes,
    undefined,
 ...
 */
export function networkJavaEnvironment(directFallback = false): Promise<NodeJS.ProcessEnv> {
 ...
 */
export function automaticMinecraftNetwork(): boolean {
 ...
 */
export function isMinecraftNetworkFailure(output: string): boolean {
  if (
    /Compilation failed|\bcompile(?:Java|Kotlin) FAILED|\berror: (?:cannot find symbol|incompatible types)|Build cancelled/iu.test(
      output,
    )
  )
    return false
  if (/MalformedJsonException:[^\n]* path \$\.versions\[/u.test(output)) return true
  if (/net\.fabricmc\.loom\.util\.download\.DownloadException: Failed to download/u.test(output)) return true
  return (
    /UnknownHostException|ConnectException|SocketTimeoutException|Connection (?:timed out|reset|refused)/iu.test(
      output,
    ) ||
 ...
 */
export async function networkGradleArguments(loader: string, officialOnly = false): Promise<string[]> {
  if (officialOnly || scope.getStore()?.settings.mode === 'proxy') return ['-Ddsh.bootstrap.officialOnly=true']
