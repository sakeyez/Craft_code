...
import { extractZipFile } from "./archive.js";
/**
 ...
 */
export const cacheRoot = () => join(process.env.DSH_HOME ?? join(homedir(), '.dsh'), 'cache', 'minecraft-workbench');
/**
 ...
 */
export function requiredJava(version) {
    if (/^1\.20\.[1-4]$/u.test(version))
        return 17;
    if (/^1\.20\.[5-6]$|^1\.21(?:\.\d+)?$/u.test(version))
        return 21;
 ...
 */
export async function probe(ctx, executable, major, signal, managed) {
    try {
        ...
        if (result.exitCode === 0 && Number(found?.[1]) === major)
            return { executable, major, managed, available: true };
    }
    catch {
 ...
 */
export async function findJava(ctx, major, signal = new AbortController().signal) {
 ...
 */
export async function ensureJava(ctx, major, signal, progress, bytes) {
    const existing = await findJava(ctx, major, signal);
    if (existing.available)
        return existing;
    return installJava(ctx, major, signal, progress, bytes);
}
async function installJava(ctx, major, signal, progress, bytes) {
    const os = process.platform === 'win32' ? 'windows' : process.platform === 'darwin' ? 'mac' : 'linux';
    const arch = process.arch === 'x64' ? 'x64' : process.arch === 'arm64' ? 'aarch64' : undefined;
    if (!arch)
        throw new Error('当前架构不支持自动安装 Java，请手动配置 JDK。');
    await progress(`正在获取 Adoptium JDK ${major} (${os}/${arch})。\n`);
    const json = await fetchJson(`https://api.adoptium.net/v3/assets/latest/${major}/hotspot?architecture=${arch}&image_type=jdk&os=${os}&vendor=eclipse`, signal);
    ...
    if (os === 'windows')
        await extractZipFile(archive, destination, signal);
    else {
        await untar({
            file: archive,
            cwd: destination,
            strict: true,
            preservePaths: false,
            filter: (path, entry) => !path.split('/').includes('..') &&
                !path.startsWith('/') &&
                'type' in entry &&
    ...
    for (const root of roots.filter(row => row.isDirectory())) {
        const executable = join(destination, root.name, ...(os === 'mac' ? ['Contents', 'Home'] : []), 'bin', os === 'windows' ? 'java.exe' : 'java');
