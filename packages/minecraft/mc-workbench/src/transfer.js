...
import { networkTransfer } from "./network.js";
 ...
 */
export async function downloadArtifact(request) {
    ...
    if (transfer && transfer.identity !== identity)
        throw new Error('缓存文件身份冲突。');
    if (!transfer) {
        const controller = new AbortController();
        const consumers = new Set();
        transfer = { controller, consumers, done: Promise.resolve(''), identity };
        const owned = transfer;
        transfer.done = transferFile({ ...request, destination, signal: controller.signal }, identity, (received, total) => {
            for (const notify of consumers)
                notify(received, total);
        }).finally(() => {
            if (active.get(key) === owned)
                active.delete(key);
        });
        active.set(key, transfer);
    }
    const owned = transfer;
    const notify = (received, total) => {
        try {
    ...
    return new Promise((accept, reject) => {
        ...
        void owned.done
            ...
            if ((await fileDigest(file, algorithm)) !== value.toLowerCase())
                throw new Error('缓存文件身份冲突。');
            if ((await stat(file)).size > request.maxBytes)
...
async function transferFile(request, identity, progress) {
    ...
    if ((await fileDigest(destination, digest.algorithm).catch(() => '')) === digest.value.toLowerCase()) {
        if ((await stat(destination)).size <= maxBytes)
            return destination;
    }
    ...
    await networkTransfer(request.url, { ...(signal ? { signal } : {}), headers: { 'User-Agent': 'CraftCode' } }, maxBytes, async (response) => {
        ...
        if (response.status !== 206)
            await reset();
        const total = response.status === 206 && range
            ? Number(range[3])
