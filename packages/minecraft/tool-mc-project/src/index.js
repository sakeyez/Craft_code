...
import { datagenTaskCandidates, loaderSupport, validationCommands } from "./loader-support.js";
import { parseGradleTaskNames, runtimeTaskCandidates, hasMinecraftReadiness } from "./gradle-tasks.js";
import { errorCode, isAbortedError, listOptionalDir, optionalStat, readBoundedText, readOptionalText, readTextFile, sessionResolveOptions, walkFiles, } from "./fs-support.js";
/**
 * Cordis plugin name.
 */
...
function isApprovedRuntimeCheck(exec) {
...
const DATA_JSON_FOLDERS = ['tags', ...DATA_JSON_FOLDER_PAIRS.flat()];
...
function addGradleTask(state, task) {
    const normalized = task.trim().replace(/^:/u, '');
    if (normalized === '' || !/^[A-Za-z][A-Za-z0-9:_-]*$/u.test(normalized))
        return;
    pushUnique(state.gradleTaskCandidates, normalized);
}
function scanGradleTaskDeclarations(state, text) {
...
function addMinecraftVersion(state, value, source, evidence) {
...
function addMapping(state, type, version, source, evidence) {
    state.mappings.push({
        type,
...
function arrayAt(record, key) {
    return typeof record === 'object' && record !== null && Array.isArray(record[key])
        ? record[key]
        : undefined;
}
function scanGradleText(state, file) {
    scanGradleTaskDeclarations(state, file.text);
    for (const pattern of LOADER_PATTERNS) {
        if (pattern.regex.test(file.text))
            addLoader(state, pattern.loader, `${file.path}: ${pattern.label}`);
    }
    const minecraftPropertyPattern = /^\s*(?:minecraft[_-]?version|minecraft_version|minecraftVersion)\s*=\s*["']?([^"'\s#]+)["']?/gimu;
    for (const match of file.text.matchAll(minecraftPropertyPattern)) {
        const value = concreteVersion(match[1]);
        if (value !== undefined)
            addMinecraftVersion(state, value, file.path, 'Minecraft version property');
    }
    const minecraftDeclarationPattern = /(?:com\.mojang:minecraft:|minecraft_version\s*=\s*["']|minecraftVersion\s*=\s*["'])([^"'\s)]+)/gimu;
    for (const match of file.text.matchAll(minecraftDeclarationPattern)) {
        const value = concreteVersion(match[1]);
        if (value !== undefined)
            addMinecraftVersion(state, value, file.path, 'Minecraft dependency/version declaration');
    }
    ...
    for (const match of file.text.matchAll(/net\.fabricmc:yarn:([^"'\s)]+)/gimu)) {
        const value = concreteVersion(match[1]);
        if (value !== undefined)
            addMapping(state, 'yarn', value, file.path, 'Yarn mappings dependency');
    }
    ...
    for (const match of file.text.matchAll(/org\.parchmentmc\.data:parchment-[^:]+:([^"'\s)]+)/gimu)) {
        const value = concreteVersion(match[1]);
        if (value !== undefined)
            addMapping(state, 'parchment', value, file.path, 'Parchment mappings dependency');
    }
...
function scanCustomResourceRoots(state, text) {
...
function scanVersionsToml(state, file) {
    let document;
    try {
    ...
    for (const key of ['minecraft', 'minecraft_version', 'minecraftVersion']) {
        ...
        if (typeof value === 'string')
            addMinecraftVersion(state, value, file.path, `versions.${key}`);
    }
    for (const [key, type] of [['yarn', 'yarn'], ['parchment', 'parchment'], ['mappings', 'unknown']]) {
        const value = versions[key];
        if (typeof value === 'string')
            addMapping(state, type, value, file.path, `versions.${key}`);
    }
}
function parseMetadata(state, file, format) {
    let document;
    try {
        document = format === 'JSON' ? JSON.parse(file.text) : parseToml(file.text, { integersAsBigInt: false });
    }
    catch (error) {
...
function scanFabricMetadata(state, file) {
    ...
    for (const mixin of mixins) {
        if (typeof mixin === 'string')
            state.mixins.push({ path: mixin, source: `${file.path}: mixins` });
        else if (typeof mixin === 'object' && mixin !== null) {
            const config = stringAt(mixin, 'config');
            if (config !== undefined)
                state.mixins.push({ path: config, source: `${file.path}: mixins.config` });
        }
    }
    ...
    if (minecraft !== undefined)
        addMinecraftVersion(state, minecraft, file.path, 'depends.minecraft');
}
function scanQuiltMetadata(state, file) {
    const document = parseMetadata(state, file, 'JSON');
    if (document === undefined)
        return;
    ...
    for (const mixin of mixins) {
        if (typeof mixin === 'string')
            state.mixins.push({ path: mixin, source: `${file.path}: quilt_loader mixins` });
    }
}
function scanTomlMetadata(state, file) {
    const document = parseMetadata(state, file, 'TOML');
    if (document === undefined)
        return;
    const loader = posix.basename(file.path) === 'neoforge.mods.toml' ? 'neoforge' : 'forge';
    addLoader(state, loader, `${file.path}: ${posix.basename(file.path)}`);
    const mods = arrayAt(document, 'mods') ?? [];
    for (const mod of mods)
        addModId(state, stringAt(mod, 'modId'), `${file.path}: [[mods]].modId`, 'high');
}
function scanSourceText(state, file) {
...
function chooseMinecraftVersion(state) {
    const candidates = uniq(state.minecraftVersions, item => `${item.value}\0${item.classification}\0${item.source}\0${item.evidence}`);
    ...
    if (exactValues.length === 1) {
        ...
        return { status: 'determined', value: rangeValues[0], classification: 'range', candidates };
    }
...
function rangeIncludesExact(range, exact) {
...
function chooseMappings(state) {
    const candidates = uniq(state.mappings, mapping => `${mapping.type}\0${mapping.version ?? ''}\0${mapping.source}\0${mapping.evidence}`);
    if (candidates.length === 0)
        return { status: 'unknown', candidates: [] };
...
function summarizeText(text, alreadyTruncated, spillPath, maxBytes) {
...
function summarizeStream(output, maxBytes) {
...
function commandStep(step, command, result, config) {
    return {
        step,
        command,
...
function runtimeProbeStep(step, mode, ready) {
...
function failedStaticStep(step, message, detail, config) {
    return {
        step,
        status: 'failed',
        exitCode: null,
...
function passedStaticStep(step, message) {
    return {
        step,
        status: 'passed',
        exitCode: 0,
...
function unavailableStep(step, message) {
    return {
        step,
        status: 'failed',
        exitCode: null,
...
async function gradleLauncher(ctx, exec, detected) {
    if (detected.inspected.gradleFiles.length === 0)
        return null;
...
function addTargetPlans(plans, target, detected) {
    switch (target) {
        case 'build':
        ...
        case 'test':
            ...
            return undefined;
        case 'datagen': {
            if (detected.loaderSupport !== 'supported')
                return 'datagen';
        ...
        case 'startup':
            ...
            if (detected.datagenClues.length > 0)
                plans.push({ step: 'datagen' });
            plans.push({ step: 'resources:gradle', task: 'processResources' }, { step: 'test', task: 'test' }, { step: 'build', task: 'build' }, { step: 'runtime' });
            return undefined;
        case 'all': {
            if (detected.datagenClues.length > 0) {
                if (detected.loaderSupport === 'supported')
                ...
                if (detected.loaderSupport === 'unknown')
                    return 'datagen';
            }
            plans.push({ step: 'resources:gradle', task: 'processResources' }, { step: 'test', task: 'test' }, { step: 'build', task: 'build' });
            return undefined;
        }
    }
}
function suggestedNextAction(step, result) {
...
function commandFor(launcher, task) {
    return `${launcher} ${task}`;
}
function taskDiscoveryFailure(step, message) {
    return { ...step, status: 'failed', message };
}
async function listGradleTasks(ctx, exec, config, launcher, timeoutMs, cache) {
...
async function discoverDatagenTask(ctx, exec, config, detected, launcher, timeoutMs, cache) {
    ...
    if (staticCandidates.length > 1) {
        return {
            step: unavailableStep('datagen', `Multiple datagen Gradle tasks were declared: ${staticCandidates.join(', ')}. Inspect the project and choose one explicitly.`),
        };
    }
    const listing = await listGradleTasks(ctx, exec, config, launcher, timeoutMs, cache);
    if (listing.step.status === 'failed')
        return { step: listing.step };
    const candidates = datagenTaskCandidates(detected.loader, listing.tasks);
    if (candidates.length === 1) {
        const task = candidates[0];
        /* v8 ignore next -- length one guarantees a defined candidate. */
        if (task !== undefined)
            return { task, step: listing.step };
    }
    if (candidates.length === 0) {
...
async function discoverRuntimeTask(ctx, exec, config, detected, launcher, mode, timeoutMs, cache) {
    ...
    if (staticCandidates.length > 1) {
        return {
            step: unavailableStep('runtime', `Multiple ${mode} runtime Gradle tasks were declared: ${staticCandidates.join(', ')}. Inspect the project and choose one explicitly.`),
        };
    }
    const listing = await listGradleTasks(ctx, exec, config, launcher, timeoutMs, cache);
    if (listing.step.status === 'failed')
...
async function unsupportedGradleLayout(ctx, exec, config) {
...
async function runCommandStep(ctx, exec, config, plan, launcher, timeoutMs, runtimeMode) {
    if (plan.task === undefined)
        throw new Error(`unresolved Gradle task for ${plan.step}`);
...
async function runStaticResourceStep(ctx, exec, config, scan) {
    const validation = await validateResources(ctx, exec, config, scan);
    if (validation.errors.length > 0) {
        return failedStaticStep('resources:static', `Minecraft resource validation reported ${validation.errors.length} error(s).`, {
    ...
    if (!validation.scanComplete) {
        return failedStaticStep('resources:static', 'Minecraft resource validation was incomplete because the scan budget or a declared resource root was not fully inspected.', { warnings: validation.warnings, checkedFiles: validation.checkedFiles, detectedModId: validation.detectedModId }, config);
    }
    const step = passedStaticStep('resources:static', `validated ${validation.checkedFiles.length} Minecraft resource file(s); warnings: ${validation.warnings.length}`);
...
function sortValidation(result) {
    return {
...
function addResourceIssue(list, code, path, message, reference = null, expectedPath = null) {
    list.push({ code, path, message, reference, expectedPath });
}
function parseResourceLocation(reference) {
    if (reference.startsWith('#') || !RESOURCE_LOCATION_PATTERN.test(reference))
        return undefined;
    const colon = reference.indexOf(':');
    if (colon >= 0)
        return { namespace: reference.slice(0, colon), path: reference.slice(colon + 1) };
    return { namespace: 'minecraft', path: reference };
}
function shouldCheckLocalReference(namespace, currentNamespace, detectedModId) {
    return namespace !== 'minecraft' && (namespace === currentNamespace || namespace === detectedModId);
}
function texturePath(root, namespace, path) {
    return posix.join(root, `assets/${namespace}/textures/${path}.png`);
}
function modelPath(root, namespace, path) {
...
function pngCrc(bytes) {
    ...
    for (const byte of bytes)
        value = (value >>> 8) ^ (PNG_CRC_TABLE[(value ^ byte) & 0xff] ?? 0);
    return (value ^ 0xffffffff) >>> 0;
}
function pngError(bytes) {
    const signature = [0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a];
    if (bytes.length === 0)
        return 'PNG file is empty';
    ...
    while (offset < bytes.length) {
        ...
        if (end > bytes.length)
            return `PNG ${type || 'chunk'} data is truncated`;
        ...
        if (expectedCrc !== actualCrc)
            return `PNG ${type || 'chunk'} CRC is invalid`;
        if (!hasHeader && type !== 'IHDR')
            return 'PNG must begin with an IHDR chunk';
        if (type === 'IHDR') {
            ...
            if (header.getUint32(0) === 0 || header.getUint32(4) === 0)
    ...
    if (!hasHeader)
        return 'PNG has no IHDR chunk';
    if (!hasEnd)
        return 'PNG has no complete IEND chunk';
    return undefined;
}
async function validatePngFiles(ctx, exec, root, namespace, config, result) {
    const warnings = [];
    const files = await walkFiles(ctx, exec, posix.join(root, `assets/${namespace}/textures`), { entries: 0, warned: false }, config, warnings, path => path.endsWith('.png'));
    for (const warning of warnings)
        addResourceIssue(result.warnings, 'scan_warning', root, warning);
    for (const file of files) {
        ...
        if (file.entry.size !== undefined && file.entry.size > config.maxFileBytes) {
            ...
            addResourceIssue(result.warnings, 'file_too_large', file.path, `Skipped because file size ${file.entry.size} exceeds maxFileBytes ${config.maxFileBytes}`);
            continue;
        }
        try {
            ...
            if (error !== undefined)
                addResourceIssue(result.errors, 'invalid_png', file.path, error);
        }
        catch (error) {
            if (isAbortedError(error, exec.signal))
                throw error;
            if (errorCode(error) === 'FS_TOO_LARGE') {
                addResourceIssue(result.warnings, 'file_too_large', file.path, `Skipped because content exceeds maxFileBytes ${config.maxFileBytes}`);
                continue;
            }
            addResourceIssue(result.errors, 'invalid_png', file.path, `PNG could not be read as bounded binary data: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
}
function validateLanguageDocuments(documents, result) {
    for (const file of documents) {
        ...
        for (const [key, value] of Object.entries(entries)) {
            if (typeof value !== 'string') {
                addResourceIssue(result.errors, 'invalid_lang_entry', file.path, `Language entry ${key} must have a string value.`, key);
            }
        }
    }
}
function parseJsonForValidation(result, path, text) {
    result.checkedFiles.push(path);
    try {
        return JSON.parse(text);
    }
    catch (error) {
        addResourceIssue(result.errors, 'invalid_json', path, 
        /* v8 ignore next -- JSON.parse throws SyntaxError, never a non-Error value. */
        `JSON parse failed: ${error instanceof Error ? error.message : String(error)}`);
        return undefined;
    }
}
async function readValidationText(ctx, exec, path, entry, config, result) {
    if (entry.size !== undefined && entry.size > config.maxFileBytes) {
        addResourceIssue(result.warnings, 'file_too_large', path, `Skipped because file size ${entry.size} exceeds maxFileBytes ${config.maxFileBytes}`);
        return undefined;
    }
    try {
        ...
        if (isAbortedError(error, exec.signal))
            throw error;
        if (errorCode(error) === 'FS_TOO_LARGE') {
            addResourceIssue(result.warnings, 'file_too_large', path, `Skipped because content exceeds maxFileBytes ${config.maxFileBytes}`);
            return undefined;
        }
        addResourceIssue(result.warnings, 'read_failed', path, 
...
function collectObjectModels(value, out) {
    if (Array.isArray(value)) {
        for (const item of value)
            collectObjectModels(item, out);
        return;
    }
...
function collectNamespaceReferences(value, out) {
    if (typeof value === 'string') {
    ...
    if (Array.isArray(value)) {
        for (const item of value)
            collectNamespaceReferences(item, out);
        return;
    }
    const record = objectRecord(value);
    if (record === undefined)
...
function detectedHighConfidenceModId(detected, result) {
    ...
    if (candidates.length > 1) {
        addResourceIssue(result.warnings, 'ambiguous_modid', '', `Multiple high-confidence mod ids were detected: ${candidates.join(', ')}`);
    }
    return null;
}
async function fallbackResourceRoots(ctx, exec, warnings) {
    const roots = [];
    ...
    for (const entry of srcEntries.filter(candidate => candidate.type === 'directory')) {
        ...
        if ((await optionalStat(ctx, exec, path))?.type === 'directory')
            pushUnique(roots, path);
    }
...
async function detect(ctx, exec, config) {
    ...
    for (const file of gradleFiles) {
        if (file.path === 'gradle/libs.versions.toml')
            scanVersionsToml(state, file);
        else
            scanGradleText(state, file);
    ...
    for (const entry of srcEntries.filter(candidate => candidate.type === 'directory')) {
        ...
        if (java.length + kotlin.length + resources.length === 0)
            continue;
        sourceSets.push({ name: sourceSet, java, kotlin, resources });
        for (const path of java)
            pushUnique(sourceRoots, path);
        for (const path of kotlin)
            pushUnique(sourceRoots, path);
        for (const path of resources)
            pushUnique(resourceRoots, path);
    }
    for (const path of state.customResourceRoots) {
        ...
        if (info?.type === 'directory')
            ...
            warnings.push(`Gradle resource directory ${path} was declared but could not be inspected`);
    }
    const metadataFiles = [];
    const metadataWarnings = [];
    const resourceWalkState = { entries: 0, warned: false };
    const resourceClueFiles = [];
    for (const resourceRoot of resourceRoots) {
        const files = await walkFiles(ctx, exec, resourceRoot, resourceWalkState, config, metadataWarnings, path => METADATA_BASENAMES.has(posix.basename(path)) || isMixinFile(path));
        resourceClueFiles.push(...files);
        for (const file of files) {
            if (!METADATA_BASENAMES.has(posix.basename(file.path)))
                continue;
            const text = await readTextFile(ctx, exec, file.path, file.entry.target, file.entry.size, config, metadataWarnings);
            if (text !== undefined)
                metadataFiles.push(text);
        }
    }
    ...
    for (const file of resourceClueFiles) {
        if (isMixinFile(file.path))
            state.mixins.push({ path: file.path, source: 'resource file name' });
    }
    const sourceWalkState = { entries: 0, warned: false };
    let hasJava = false;
    let hasKotlin = false;
    for (const sourceRoot of sourceRoots) {
        if (sourceRoot.endsWith('/java'))
            hasJava = true;
        if (sourceRoot.endsWith('/kotlin'))
            hasKotlin = true;
    ...
    for (const file of gradleFiles) {
        if (/\b(?:org\.jetbrains\.kotlin\.jvm|kotlin\(["']jvm["']\))\b/u.test(file.text))
            hasKotlin = true;
    }
    ...
    if (gradleFiles.length === 0)
        warnings.push('no root Gradle files were found');
    if (metadataFiles.length === 0)
        warnings.push('no Minecraft mod metadata files were found under resource roots');
    if (resourceRoots.length === 0)
        warnings.push('no src/<sourceSet>/resources roots were found');
    if (loader === 'unknown' && state.loaderEvidence.size === 0)
        warnings.push('loader could not be identified from Gradle files or mod metadata');
    ...
    if (minecraftVersion.status === 'unknown')
        warnings.push('Minecraft version could not be identified');
    if (mappings.status === 'unknown')
        warnings.push('mappings could not be identified');
    ...
    const project = {
        ...
        mixinConfigs,
        datagenClues,
        recommendedValidationCommands: validationCommands(loader, loaderSupportState, gradleFiles.length > 0, hasWrapper, datagenClues, state.gradleTaskCandidates),
...
async function resourceFileExists(ctx, exec, roots, expected) {
    for (const root of roots) {
        if (await localFileExists(ctx, exec, expected(root)))
            return true;
    }
    return false;
}
async function readJsonResourceFiles(ctx, exec, base, config, result) {
    const scanWarnings = [];
    const files = await walkFiles(ctx, exec, base, { entries: 0, warned: false }, config, scanWarnings, path => path.endsWith('.json'));
    for (const warning of scanWarnings)
        addResourceIssue(result.warnings, 'scan_warning', base, warning);
    const documents = [];
    for (const file of files) {
        ...
        if (document !== undefined)
            documents.push({ path: file.path, document });
    }
    return documents;
}
async function validateModelTextures(ctx, exec, roots, root, namespace, folder, config, detectedModId, result) {
    ...
    for (const file of documents) {
        ...
        if (parent !== undefined) {
            ...
            if (parsedParent !== undefined && shouldCheckLocalReference(parsedParent.namespace, namespace, detectedModId)) {
                ...
                const parentExists = await resourceFileExists(ctx, exec, roots, candidateRoot => modelPath(candidateRoot, parsedParent.namespace, parsedParent.path));
                if (!parentExists) {
                    addResourceIssue(result.errors, 'missing_model_parent', file.path, `Model references missing local parent ${parent}`, parent, expectedParent);
                }
            }
        }
        ...
        for (const texture of Object.values(textureRecord)) {
            ...
            if (parsed === undefined)
                continue;
            if (!shouldCheckLocalReference(parsed.namespace, namespace, detectedModId))
                continue;
            ...
            if (!await resourceFileExists(ctx, exec, roots, candidateRoot => texturePath(candidateRoot, parsed.namespace, parsed.path))) {
                addResourceIssue(result.errors, 'missing_texture', file.path, `Model references missing texture ${texture}`, texture, expectedPath);
            }
        }
    }
}
async function validateBlockstateModels(ctx, exec, roots, root, namespace, config, detectedModId, result) {
    ...
    for (const file of documents) {
        ...
        for (const model of models) {
            ...
            if (!await resourceFileExists(ctx, exec, roots, candidateRoot => modelPath(candidateRoot, parsed.namespace, parsed.path))) {
                addResourceIssue(result.errors, 'missing_model', file.path, `Blockstate references missing model ${model}`, model, expectedPath);
            }
        }
    }
}
async function validateDataJson(ctx, exec, root, namespace, folder, version, config, allowedNamespaces, detectedModId, result) {
    if (detectedModId !== null && namespace !== detectedModId && !BUILTIN_RESOURCE_NAMESPACES.has(namespace)) {
        addResourceIssue(result.warnings, 'suspicious_namespace', posix.join(root, `data/${namespace}/${folder}`), `Data namespace ${namespace} differs from detected mod id ${detectedModId}`, namespace);
    }
    ...
    if (pair !== undefined && documents.length > 0) {
        ...
        if (exactVersion === null) {
            ...
            if (folder !== expectedFolder) {
                addResourceIssue(result.errors, 'data_directory_version', base, `Minecraft ${exactVersion} uses the ${expectedFolder} data directory; files under ${folder} are not loaded from this location.`, null, posix.join(root, `data/${namespace}/${expectedFolder}`));
            }
        }
    }
    for (const file of documents) {
        if (objectRecord(file.document) === undefined) {
            addResourceIssue(result.errors, 'invalid_resource_shape', file.path, `${folder} JSON must contain an object at the root.`);
            continue;
        }
        ...
        for (const reference of references) {
            ...
            if (parsed === undefined || allowedNamespaces.has(parsed.namespace))
                continue;
            addResourceIssue(result.warnings, 'suspicious_namespace', file.path, `Reference uses namespace ${parsed.namespace}, which was not detected in this project`, reference);
        }
    }
}
async function validateItemDefinitions(ctx, exec, root, namespace, config, version, result) {
    ...
    if (version.status === 'determined' && version.classification === 'exact' && valid(version.value, { loose: true }) !== null) {
        if (!gte(version.value, '1.21.4', { loose: true })) {
            addResourceIssue(result.warnings, 'item_definition_version', base, `Item definition resources are newer than detected Minecraft ${version.value}; verify the target version and resource format.`);
        }
        return;
    }
    addResourceIssue(result.warnings, 'item_definition_version_unknown', base, 'Item definition resources were found, but the exact Minecraft version is not determined; resource-format compatibility was not assumed.');
}
async function validateResources(ctx, exec, config, scan) {
    ...
    for (const path of detected.inspected.metadataFiles)
        result.checkedFiles.push(path);
    ...
    for (const root of roots) {
        ...
        for (const namespace of rootAssetNamespaces) {
            if (detectedModId !== null && namespace !== detectedModId && namespace !== 'minecraft') {
                addResourceIssue(result.errors, 'modid_mismatch', posix.join(root, `assets/${namespace}`), `Asset namespace ${namespace} differs from detected mod id ${detectedModId}`, namespace);
            }
        ...
        for (const namespace of rootDataNamespaces) {
            for (const folder of DATA_JSON_FOLDERS) {
                await validateDataJson(ctx, exec, root, namespace, folder, detected.minecraftVersion, config, allowedNamespaces, detectedModId, result);
            }
        }
    }
    result.scanComplete = result.scanComplete && !result.warnings.some(issue => issue.code === 'scan_warning' || issue.code === 'file_too_large');
...
async function runMcCheck(ctx, exec, config, args) {
    ...
    for (const unresolvedPlan of plans.filter(plan => plan.task === undefined)) {
        const discovery = unresolvedPlan.step === 'datagen'
            ...
            : await discoverRuntimeTask(ctx, exec, config, detected, launcher, args.runtimeMode, args.timeoutMs, taskDiscoveryCache);
        if (discovery.step !== undefined) {
            if (discovery.step.command === undefined || !discoveryCommands.includes(discovery.step.command)) {
                ...
                if (discovery.step.command !== undefined)
                    discoveryCommands.push(discovery.step.command);
            }
        }
...
export function apply(ctx, rawConfig = {}) {
    ...
    ctx.effect(() => {
        const dispose = ctx.on('tools/pre-execute', async (exec, next) => {
            if (!isApprovedRuntimeCheck(exec))
                return next();
            return {
                kind: 'ask',
    ...
    ctx.effect(() => {
        const fiber = ctx.inject(['shell'], (shellCtx) => {
            shellCtx.tools.register(defineTool({
                ...
                execute: (args, exec) => {
                    ...
                    if (runtime && cwd && args.runtimeMode && (args.target === 'runtime' || args.target === 'startup')) {
                        return runtime.check(cwd, args.runtimeMode, exec.signal, args.timeoutMs, args.testMode);
                    }
                    if (args.testMode === 'artifact')
                        throw new Error('成品模式需要 Minecraft 工作台及 runtime/startup 目标。');
 ...
 */
export async function inspectMinecraftProject(ctx, cwd, signal) {
 ...
 */
export async function validateMinecraftProject(ctx, cwd, signal) {
