...
包按组置于 `packages/<group>/<pkg>/`；包名仍为 `@deepseek-ai/dsh-<pkg>`。**组 README 负责包与 ctx 键映射。**
...
| [`minecraft/`](minecraft/README.zh.md) | Minecraft 领域 agent 包，包括 Fabric/NeoForge 模组提示词指导与项目检测 | 产品：稳定 API |
...
| [`host/`](host/README.zh.md) | 桌面宿主半侧：API 网关 + 回环 HTTP 路由服务器 | 产品：稳定 API |
| [`client/`](client/README.zh.md) | 桌面 renderer 客户端半侧：shell、协议层、对象服务、slot、`ui-*` 插件 | 产品：稳定 API |
