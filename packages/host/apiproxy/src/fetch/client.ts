...
export interface IApiClient {
  sessions: {
    ...
    annotate?(payload: RequestPayload<'session.annotate'>, signal?: AbortSignal): Promise<RpcResponse<ResponseValue<'session.annotate'>>>
...
const UNARY_VALUE_SCHEMAS: { [K in keyof RpcMethodMap]: z.ZodType<Wire<ResponseValue<K>>> } = {
  ...
  'session.annotate': sessionAnnotateValueSchema,
...
export abstract class AbstractApiClient implements IApiClient {
  ...
  readonly sessions: IApiClient['sessions'] = {
    ...
    annotate: (payload, signal) => this.callUnary('session.annotate', payload, signal),
