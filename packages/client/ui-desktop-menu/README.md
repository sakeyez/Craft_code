...
The preload bridge exposes closed menu and game-event subscriptions, fixed `openMenu` and active-project requests, and one `invokeProjectCommand` request/response method. The main process validates popup sender identity, menu id, content-relative integer coordinates, and project-command kinds. Ordinary command completion has one owner: the invoke result; a long-lived Minecraft process reports its later natural or failed exit through the dedicated game event. Components receive menu presentation, workspace creation, project search, and command calls through their injected faces; they never read Cordis Context or Node APIs. Electron's main process retains filesystem and process authority, while directory, artifact, and save-location choices remain native operating-system dialogs.
