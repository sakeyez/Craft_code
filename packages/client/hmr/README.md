...
Hot reload for script-loaded client plugins. The web bundle mounts the row unconditionally; without a rebuild watcher (`pnpm run dev:desktop-renderer`) rewriting client bundles, the poll observes no changes and the chain stays idle.
