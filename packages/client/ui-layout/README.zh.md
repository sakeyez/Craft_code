...
游戏 slot 将原生 Ctrl+Shift+P 订阅绑定到当前会话，并在会话变化时解除。快捷键请求和标注按钮共用同一事务；注册失败在游戏面板显示。

游戏伴随面板的控制区按内容占高，上限为对话列的 35%，仅失败或断开时显示重连。已保存标注由对话输入框呈现；游戏模式下，空会话输入框也位于底部，切换布局不重建输入框。
