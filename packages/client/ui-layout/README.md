...
The same transient store keeps strict external-game states by normalized project path. AppFrame always keeps the ordinary sidebar, conversation, and details layout; the selected project's `game` slot adds a compact companion status bar for `starting`, `connected`, `failed`, `disconnected`, `reconnecting`, or `unsupported`. Events from another project cannot change the selected project's controls. On Windows the host follows Minecraft as a separate top-level window; Linux and macOS keep the ordinary layout and explain that Minecraft runs externally. The injected `ctx.layout` operations reconnect or reposition the companion and bracket annotation capture; the returned JPEG exists only in component state, while session annotation events retain normalized geometry and descriptions.
