...
Groups hold `packages/<group>/<pkg>/`; names stay `@deepseek-ai/dsh-<pkg>`. **Group READMEs own package and ctx-key maps.**
...
| [`minecraft/`](minecraft/README.md) | Minecraft-domain agent packages, including Fabric/NeoForge mod prompt guidance and project detection | Product — stable API |
...
| [`host/`](host/README.md) | Desktop host half: API gateway + loopback HTTP route server | Product — stable API |
| [`client/`](client/README.md) | Desktop renderer client half: shell, wire, object services, slots, `ui-*` plugins | Product — stable API |
...
New packages join groups; new groups update their README and table.

## Dependencies

Dependency graph is generated: [docs/module-graph.md](../docs/module-graph.md) (`pnpm run gen-module-graph`, freshness-gated in CI).
