/**
 * The Minecraft NeoForge bundle is a static profile patch layer: it selects
 * the shipped NeoForge preset and contributes the Java LSP host rows.
 */
...
function loadPatch(): PatchRow[] {
  ...
  if (!Array.isArray(parsed)) throw new TypeError('minecraft-neoforge patch must parse to a patch list')
  return parsed as PatchRow[]
}

describe('dsh-minecraft-neoforge bundle', () => {
  ...
  it('selects the NeoForge preset and configures Java through stdio LSP', () => {
    ...
    expect(presetPatch?.config).toMatchObject({ default: 'minecraft-neoforge' })
  ...
  it('composes without warnings after base and web-app bundle layers', () => {
    ...
    expect(byId.get('agent-presets')?.config).toMatchObject({ default: 'minecraft-neoforge' })
