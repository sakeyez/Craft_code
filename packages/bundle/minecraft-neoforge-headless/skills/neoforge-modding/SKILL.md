...
Prefer the project's own Gradle wrapper. Useful checks usually include `./gradlew build`, project tests, `./gradlew runData`, or a focused datagen task. Running a client, downloading dependencies, or starting a long Gradle task can be expensive; explain that before starting it.
