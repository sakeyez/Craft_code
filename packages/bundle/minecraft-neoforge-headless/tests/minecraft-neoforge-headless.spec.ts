/**
 * The Minecraft NeoForge headless bundle is a static profile patch layer over
 * dsh-base and dsh-headless: it keeps the one-shot runner, adds NeoForge/LSP
...
describe('dsh-minecraft-neoforge-headless bundle', () => {
  it('declares a parseable patch list through the dsh.bundle.patch manifest field', () => {
    ...
    expect(manifest.dependencies).toMatchObject({
      ...
      '@deepseek-ai/dsh-minecraft-neoforge-agent': 'workspace:^',
      '@deepseek-ai/dsh-tool-lsp': 'workspace:^',
    })
  })

  it('adds NeoForge prompt, bundled skill, Java LSP, and LSP tool rows', () => {
    const rows = flatten(loadPatch())
    const byId = new Map(rows.map(row => [row.id, row]))

    expect(byId.get('system-prompt')?.config).toMatchObject({
      persona: expect.stringContaining('Minecraft NeoForge mod-development coding agent') as string,
    })
    expect(byId.get('minecraft-neoforge-agent')).toMatchObject({
      name: '@deepseek-ai/dsh-minecraft-neoforge-agent',
    })
  ...
  it('composes without warnings after base and headless bundle layers', () => {
    ...
    expect(byId.get('minecraft-neoforge-agent')?.name).toBe('@deepseek-ai/dsh-minecraft-neoforge-agent')
