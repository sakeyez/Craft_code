...
import { extractSnapshotSpillPaths, normalizeSessionLog, normalizeStdout, scrubRequestHeaders, scrubSessionSnapshot, scrubSystemPrompts, scrubToolSchemas, tokenizeSessionFixtureCwd, } from "./normalize.js";
            ...
            it.skipIf(scenarioSkipped(scenario, RECORDING, process.platform, options.hasPwsh))(`snapshot: ${scenario.name} matches the expected outputs`, async ({ expect }) => {
                ...
                if (writesSessionFixtures) {
                    ...
                    const freshFixtures = REFRESHING
                        ? result.sessionLogs.map((log, index) => scrubSessionSnapshot(portableFixture(stabilizeRefreshLog(log.content, existingFixtures[index], refreshReplacements, ctx))))
                ...
                if (comparesLog) {
                    ...
                    for (let i = 0; i < fixtureFiles.length; i++) {
                        const harvested = scrubSessionSnapshot(result.sessionLogs[i].content);
                        const fixture = scrubSessionSnapshot(await readFile(join(dir, fixtureFiles[i]), 'utf8'));
