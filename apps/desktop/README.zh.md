...
标注覆盖层使用紧凑工具栏和字母标记。小型说明输入框在所选点或区域旁打开，位置限制在视口内，不使用背景模糊或暗色遮罩。
...
当前项目拥有已连接游戏和可保存标注的会话时，Ctrl+Shift+P 打开游戏标注。主进程注册系统快捷键，并检查前台窗口是 Minecraft 或 CraftCode。按键占用会在游戏面板提示，标注按钮仍可使用；长按不会重复打开事务。

所选的可见游戏保留一条预热的 10 FPS 无音频捕获流，按经过验证的 HWND 选择窗口，不生成其他窗口的缩略图。截图请求等待新帧、裁切客户区，并编码为质量 75、不超过 1920×1080 的 JPEG。捕获流失败时回退到窗口缩略图并记录日志。预加载的无边框覆盖层显示静帧，不挂起 Minecraft。完成和取消会隐藏并重置覆盖层以供复用；窗口变化或会话失效会释放覆盖层。切换项目、最小化、断开及退出会释放捕获流。标注成功后，图片保存为会话授权的附件；会话事件只保留附件引用和标注几何。

桌面日志记录目标准备、捕获路径耗时和覆盖层显示耗时。依赖构建产物的 `tests/fixtures/annotation-performance.mjs` 使用真实 Electron 窗口测量五次冷启动和三十次预热截图；它不能证明 Minecraft 帧率或与 Codex 性能持平。
