...
// Boots the real desktop composition — the dsh-base and dsh-desktop-app bundle
...
//
// Composition divergences from `dsh desktop`, all deliberate, all via include
...
/** The shipped composition under test: the dsh-base and dsh-desktop-app bundle patches over the empty profile root. */
const BASE_PATCH_PATH = join(REPO_ROOT, 'packages/bundle/base/cordis.patch.yml')
const WEB_PATCH_PATH = join(REPO_ROOT, 'packages/bundle/desktop-app/cordis.patch.yml')
...
export interface LaunchOptions {
  /**
   * Optional product overlay applied after the shipped desktop surface and before
...
export async function launchWebScaffold(options: LaunchOptions = {}): Promise<WebScaffold> {
  ...
  const webRuntimeConfig = composedRows.find(row => row.id === 'desktop-runtime')?.config as {
  ...
  const patches: PatchOptions[] = [
    ...
    // The URL line is disabled because this scaffold owns its Playwright browser.
    // Preserve the composed surface-context choice because a patch replaces
    // the row's complete config.
    { id: 'desktop-runtime', config: { printUrl: false, surfaceContext } },
...
export function realizeSeedFixture(scaffold: WebScaffold, fixtureText: string, id: string): string {
  // Substitutions occur inside JSON strings, where native Windows separators need escaping.
  const stringContent = (value: string): string => JSON.stringify(value).slice(1, -1)
  const cwdContent = stringContent(scaffold.workspaceCwd)
  const realized = fixtureText
    .split('{{sessionId}}').join(stringContent(id))
    .split('{{cwd}}').join(cwdContent)
  const fixtureCwd = (JSON.parse(realized.split('\n', 1)[0]!) as { cwd?: string }).cwd
  return fixtureCwd === undefined
    ? realized
    : realized.split(stringContent(fixtureCwd)).join(cwdContent)
}
...
export async function compareOrRefreshGolden(goldenPath: string, actual: string, mode: WebSnapshotMode): Promise<void> {
  ...
  if (!existsSync(goldenPath)) {
    throw new Error(`missing golden ${goldenPath} — run DSH_SNAPSHOT=refresh pnpm run test:desktop-renderer to generate it`)
  }
