// Real-host smoke: spawn `dsh desktop` with a real key, walk the full flow
...
// convention); vitest.desktop-renderer.config.ts loads the repo-root .env before this file
...
function waitForReadyLine(child: ChildProcess): Promise<string> {
  return new Promise((resolveReady, reject) => {
    let out = ''
    const timer = setTimeout(() => { reject(new Error(`dsh desktop not ready in 90s; output:\n${out}`)) }, 90_000)
    const onData = (chunk: Buffer): void => {
      out += chunk.toString()
      const match = /dsh desktop: (http:\/\/[^\s]+)/.exec(out)
    ...
    child.once('exit', (code) => {
      clearTimeout(timer)
      reject(new Error(`dsh desktop exited early (code ${code}); output:\n${out}`))
    })
  })
}
...
// Readiness gate: `dsh desktop` serves every production manifest plugin; until every UI
...
describe('dsh desktop keyless CLI smoke', () => {
  it('listens on 127.0.0.1 by default', async () => {
    ...
    const child = spawn(
      process.execPath,
      ['--import', tsxLoader, join(REPO_ROOT, 'apps/cli/src/bin.ts'), 'desktop', '--port', '0'],
      {
        cwd: sessionsDir,
        env: {
          ...process.env,
  ...
  it('routes desktop runtime context and workspace instructions through the real CLI request', async () => {
    requireDist()
...
describe.skipIf(!process.env.DEEPSEEK_API_KEY || notReady.length > 0)('web smoke (real host, real key)', () => {
  ...
  it('cold start: loading page settles into the three-column frame', async () => {
    ...
    expect(await page.locator('text=World load failed').count()).toBe(0)
