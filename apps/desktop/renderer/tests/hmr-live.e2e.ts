/** Published dsh desktop + pnpm dev:desktop-renderer → browser HMR, with no page reload. */
...
it('hot-reloads a real client-plugin source edit without refreshing the page', async () => {
  ...
  const oldText = 'Craft a new idea'
  const sourceNeedle = "'hero.headline': 'Craft a new idea'"
  ...
  try {
    ...
    await waitForOutput(watcher, /desktop-renderer: watching/, 'pnpm run dev:desktop-renderer')
    host = subprocessCtx.subprocess.spawn(spawnSpec(
      [process.execPath, binPath, 'desktop', '--port', '0'],
      world,
      {
    ...
    const baseUrl = await waitForOutput(host, /dsh desktop: (http:\/\/[^\s]+)/, 'built dsh desktop')
