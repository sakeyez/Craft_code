...
export function requireDist(): void {
  if (!existsSync(DIST_INDEX)) {
    throw new Error('web app dist not built — run `pnpm run build` from the repository root (`pnpm run test:desktop-renderer` does this first)')
  }
}

/** OS-assigned free port, released before use (the spawned `dsh desktop` needs a concrete --port). */
...
export async function connectFreshWorkspaceZh(page: Page, root: string, name = 'workspace'): Promise<void> {
  ...
  await page.locator('[data-composer-card] textarea:enabled:not([readonly])')
    .waitFor({ timeout: 15_000 })
}
