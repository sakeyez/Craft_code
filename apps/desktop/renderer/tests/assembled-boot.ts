...
// workspace `lib/client.js` artifacts booted through RendererBootEntry's
...
const BUNDLE_LAYERS = [
  ...
  {
    manifest: join(REPO_ROOT, 'packages/bundle/desktop-app/package.json'),
    patch: join(REPO_ROOT, 'packages/bundle/desktop-app/cordis.patch.yml'),
  },
  {
    manifest: join(REPO_ROOT, 'packages/bundle/mcmod/package.json'),
    patch: join(REPO_ROOT, 'packages/bundle/mcmod/cordis.patch.yml'),
  },
] as const
...
/** Derive the assembled browser graph from the same bundle patches and package declarations as `dsh desktop`. */
...
export function mountAssembledApp(search = '?fixture'): void {
  ...
  act(() => {
    const entry = new RendererBootEntry(root, {
