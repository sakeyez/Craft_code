...
describe('web e2e: fresh round trip through the real assembly', () => {
  ...
  it('records the desktop surface, source checkout, and session cwd in the request header', async () => {
    ...
    if (agent === undefined) throw new Error(`the settled desktop agent ${settledSessionId} is no longer live`)
  ...
  it('exposes the assembled desktop URL to the real bash tool', async () => {
    if (settledSessionId === undefined) throw new Error('the drive turn did not publish a session id')
    const agent = scaffold.ctx.agents.get(settledSessionId)
    if (agent === undefined) throw new Error(`the settled desktop agent ${settledSessionId} is no longer live`)
    const result = await scaffold.ctx.tools.execute({
      ...
      arguments: {
        command: 'printf \'%s\\n\' "$DSH_DESKTOP_URL"',
        description: 'Print current desktop runtime',
      },
      agent,
    })
