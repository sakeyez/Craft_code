...
Prefer the project's own Gradle wrapper. Useful checks usually include `./gradlew build`, project tests, `./gradlew runDatagen`, or a focused datagen task. Running a client, downloading dependencies, or starting a long Gradle task can be expensive; explain that before starting it and use background jobs when the command needs to keep running.
