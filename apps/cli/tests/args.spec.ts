...
describe('parseDshArgs', () => {
  it('routes profile boots and the desktop alias, handing the rest to the app', () => {
  ...
  it('ends the launcher flags at the first token it does not own', () => {
    ...
    expect(parse(['--profile', 'desktop', '-h']))
      .toEqual({ mode: 'profile', profile: 'desktop', patches: [], args: ['-h'] })
    expect(parse(['desktop', '--port', '8080', '--future-desktop-flag']))
      .toEqual({ mode: 'profile', profile: 'desktop', patches: [], args: ['--port', '8080', '--future-desktop-flag'] })
  ...
  it('routes profile and desktop config dumps', () => {
  ...
  it('rejects missing profile, removed flags, and contradictory inputs', () => {
    ...
    expect(exitCode(['desktop', '--dump-default-config', '--patch', 'w.yml'])).toBe(1)
    expect(exitCode(['desktop', '--patch='])).toBe(1)
    ...
    expect(exitCode(['desktop', '--dump-config', '--port', '8080'])).toBe(1)
    expect(exitCode(['--profile', 'desktop', '--dump-config', '-h'])).toBe(1)
    expect(exitCode(['web'])).toBe(1)
