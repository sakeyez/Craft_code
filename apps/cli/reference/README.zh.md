...
本参考定义 profile 启动、desktop 命令、插件管理和配置 dump 等命令模式。argv 由 [`src/args.ts`](../src/args.ts) 统一解析一次，[`src/bin.ts`](../src/bin.ts) 只会动态导入选中的运行器。
...
组合包名称先从 dsh 安装目录解析，再从 profile 目录解析。因此，内置组合包（`@deepseek-ai/dsh-base`、`@deepseek-ai/dsh-desktop-app`、`@deepseek-ai/dsh-headless`）始终来自当前运行的 `dsh` 所属的安装；树外组合包则来自 profile 中由 pnpm 管理的 `node_modules`。patch 行中的裸插件 `name` 会从 profile 目录开始，按照 Node 的模块解析规则逐级向父目录查找，直至由 dsh 维护的安装后备目录 `$DSH_HOME/profiles/node_modules`。该目录为 dsh 安装中的应用和组合包所依赖的每个包各维护一个符号链接，并在每次启动时修复这些链接。

`desktop`、`headless` 和 `mcmod` profile 首次使用时会从随附模板自动初始化。其他缺失的 profile 会显式报错，并提示运行 `dsh plugin --profile <name> add <package>`。

### 应用参数

启动器自身的 flag 必须写在最前面，并在遇到第一个无法识别的 token 时结束；从该 token 开始的所有内容都会通过 `ctx.cmdlineArgs` 原样交给已启动的 profile，注入该 profile 的任意应用插件都可以解析这些内容（[`dsh-cmdline`](../../../packages/boot/cmdline/README.zh.md)）。因此，`dsh --profile desktop --port 8080` 会将 `--port` 交给 desktop 运行时；`dsh --profile desktop --help` 只打印该应用的帮助信息，不启动应用；`dsh --help` 没有可供交付参数的 profile，因此会打印启动器自身的帮助信息。

每套组合只会挂载一次。desktop 启动插件注入 `cmdlineArgs`，解析 `--port` 并提供 `desktopStartup`；Loader 会等到该服务激活后，再对 webserver 行的配置求值（`port: !!js ctx.desktopStartup.port ?? 3080`）。帮助参数和被拒绝的参数不会激活依赖该提供方的配置行。
...
`dsh desktop` 是 `--profile desktop` 的硬编码别名；唯一的应用参数是 `--port`。运行时只绑定 loopback 并打印供 Electron 使用的 readiness URL。客户端插件 HMR（热模块替换）接收器始终挂载，在单独运行的 `pnpm run dev:desktop-renderer` watcher 重建客户端 bundle 之前保持空闲。
...
desktop 运行时需要已构建的包和 renderer 产物（`pnpm run build`）。默认服务地址是 `http://127.0.0.1:3080`，且绝不会打开外部浏览器。Electron 开发流程传入 `--port 0` 并直接加载结算后的 loopback URL。
