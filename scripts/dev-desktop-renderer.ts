 ...
 * itself (`dsh desktop`), so any process that rewrites `lib/client.js` files
 ...
 * `lib/client.js`, and `vite build` rewrites `apps/desktop/renderer/dist`, which `dsh desktop`
 ...
 * `lib/` and `apps/desktop/renderer/dist/` trees.
 *
 * Usage: `pnpm exec tsx scripts/dev-desktop-renderer.ts [--poll[=ms]]`. Requires one prior
...
/** Compile-shell workspace whose dist `dsh desktop` serves. */
const SHELL_PACKAGE = '@deepseek-ai/dsh-desktop'
...
/**
 * Start the tsdown watch build used by `pnpm run dev:desktop-renderer`.
...
function spawnStage(stage: string, command: string, args: readonly string[], local: boolean): void {
  ...
  void child.then((result) => {
    console.error(`desktop-renderer: ${stage} exited (code ${String(result.exitCode)}); the artifact chain is now stale`)
    process.exit(1)
  })
}
...
if (isMain) {
  ...
  if (pluginDirs.length === 0) {
    console.error('desktop-renderer: no dsh.client packages found under packages/')
    process.exit(1)
  }
  if (libraryDirs.length === 0) {
    console.error('desktop-renderer: no client-preset library packages found under packages/ — the compile shell links their lib products, so an empty set means the discovery predicate is stale')
    process.exit(1)
  }
  ...
  if (args.some(a => a !== pollArg)) {
    console.error('desktop-renderer: usage: tsx scripts/dev-desktop-renderer.ts [--poll[=ms]]')
    process.exit(1)
  }
  ...
  if (pollInterval !== undefined && (!Number.isInteger(pollInterval) || pollInterval <= 0)) {
    console.error(`desktop-renderer: invalid --poll interval "${pollArg ?? ''}"`)
    process.exit(1)
  }
  ...
  // running vite from anywhere but apps/desktop/renderer silently switches which react copy
  // the bundle gets.
  spawnStage('vite build --watch', 'pnpm', ['--filter', SHELL_PACKAGE, 'run', 'build:renderer', '--', '--watch'], false)

  console.log(
    `desktop-renderer: watching ${String(pluginDirs.length)} dsh.client plugin packages`
