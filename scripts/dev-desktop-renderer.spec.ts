...
import { discoverLibraryDirs, discoverPluginDirs, watchClientPlugins } from './dev-desktop-renderer.ts'

it('discovers dsh.client packages with sibling roles', async () => {
  const root = await mkdtemp(join(tmpdir(), 'dsh-dev-desktop-renderer-discovery-'))
  try {
...
it('discovers client-preset packages the shell links, excluding loader-delivered and test infrastructure', async () => {
  const root = await mkdtemp(join(tmpdir(), 'dsh-dev-desktop-renderer-library-'))
  try {
...
it('rebuilds a client-plugin bundle after its source changes', async () => {
  const root = await mkdtemp(join(tmpdir(), 'dsh-dev-desktop-renderer-watch-'))
  let bundles: TsdownBundle[] = []
  try {
    await symlink(join(import.meta.dirname, '..', 'node_modules'), join(root, 'node_modules'), 'dir')
    await writeFile(join(root, 'package.json'), JSON.stringify({ name: '@dsh-test/dev-desktop-renderer-watch', private: true, type: 'module' }))
